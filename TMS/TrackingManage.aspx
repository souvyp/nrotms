       ...
       <div class="modal-dialog" style="height:170px">
          <div class="modal-content" style="height:100%;width:64%">
                ...
                <div class="modal-footer" style="position:absolute; top:0; width:100%; padding-bottom:0;">
					<button type="button" class="btn btn-default  okButton footKeepBtn" style="margin-right:8px; box-shadow:none;text-shadow:none;" aria-label="OK" title="确定" onclick="skiptoprint($(this).closest('#printTBModal'), 2)" ><span aria-hidden="true" class="">确定</span><span class="glyphicon glyphicon-ok" style="margin-left:2px;"></span></button>
		...
		<table class="jptable  table" id="demoList">
			<thead>
				<tr class="trtitle">
					...
					<td style="width: 26%;" class="title">单据编号</td>
			...
			{{each data}}
				<tr>
					...
					<td class="title" title="{{$value.code}}" fld="code"><a target="_blank" href="TrackingManage_edit.aspx?id={{$value.id}}&code=TransOrder">{{$value.code}}</a></td>
					...
					<td class="title" title="{{$value.StatusName}}" fld="StatusName">
						{{if $value.SupplierID != "0" && $value.status == "1"}}
					...
					<td class="title"  fld="ContainsOwnerOrderCode" title="{{$value.ContainsOwnerOrderCode}}" >
						{{if $value.SupplierID == "0" && $value.status == '1'}}
							<a target="_blank" href="CombinedSend_edit.aspx?id={{$value.OwnerOrderID}}&SupplierID={{$value.SupplierID}}">{{$value.ContainsOwnerOrderCode}}</a>
						{{else}}
							<a target="_blank" href="CommissionedCar_edit.aspx?id={{$value.OwnerOrderID}}">{{$value.ContainsOwnerOrderCode}}</a>
						{{/if}}
                    </td>
                    <td class="title" fld="DeviceCode">
                    	{{if $value.DeviceCode != ''}}
							<a target="_blank" href="http://106.14.17.157/gps/map/marker.action?devCode={{$value.DeviceCode}}" style="margin-right:5px;">定位</a>
		...
		<div id="demo" class="box jplist">
			...
			<div class="jplist-panel box panel-top min_height">
				...
				<div class="jplist-drop-down" data-control-type="sort-drop-down" data-control-name="sort" data-control-action="sort">
					<ul>
						<li><span data-path="ContainsOwnerOrderCode" data-order="desc" data-type="text">排序</span></li>
                ...
                <div style="" class="jplist-drop-down" data-control-type="filter-drop-down" data-control-name="status" data-control-action="filter">
               ...
               <div class="jplist-drop-down" data-control-type="filter-drop-down" data-control-name="Combined" data-control-action="filter"><div class="jplist-dd-panel">拼车状态</div>
				...
				<button class="btn btn-default navbar-left footKeepBtn" data-toggle="modal" onclick="OrderTrackingToXml(3)" title="导出">导出</button>
				...
				<div class="collapse clearfix" id="collapseExample">
                    ...
                    <div class="text-filter-box">
                        <input data-path="SupplierName" name="SupplierName" class="data" data-button="#opt_status-search-button" type="text" value="" placeholder="承运方名称" data-control-type="textbox" data-control-name="SupplierName" data-control-action="filter">
				    </div>
                    <div class="text-filter-box">
				    ...
				    <div class="text-filter-box">
	                    <input data-path="ContainsOwnerOrderCode" name="ContainsCode" class="data" data-button="#opt_status-search-button" type="text" value="" placeholder="拼车单编号" data-control-type="textbox" data-control-name="ContainsOwnerOrderCode" data-control-action="filter" />
				    </div>
				    <div class="text-filter-box">
...
<script type="text/javascript">
    ...
    var reqeustDone = function () {		/*所有JS加载完成以后执行*/
        ...
        InitList($("#demo"), $("#demo .list"), $("#jplist-template").html(), "TrackingManage", "/Widget.aspx?param=jplist&vid=grid_jplist_0042");
