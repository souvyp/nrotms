...
<html>
<head>		
<title>已接收拼车单-<%=NDS.Kits.Config.Instance.WebsiteParameters("ProductName")%></title>
<meta charset="utf-8" />
		...
		<table class="jptable table">
			...
			{{each data}}
				<tr>
					...
					<td class="title"  fld="Code" title="{{$value.Code}}"><a href="OrderAcceptedCar_edit.aspx?id={{$value.id}}" >{{$value.Code}}</a></td>
...
<script type="text/javascript">
    var reqeustDone = function () {		/*所有JS加载完成以后执行*/
        ...
        InitList($("#demo"), $("#demo .list"), $("#jplist-template").html(), "OrderAcceptedCar", "/Widget.aspx?param=jplist&vid=grid_jplist_0068");
