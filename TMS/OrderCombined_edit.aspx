...
<html>
<head>    
<title>市内合单信息-<%=NDS.Kits.Config.Instance.WebsiteParameters("ProductName")%></title>
<meta charset="utf-8" />
...
<script language="javascript">
    var __saveNdtCfg = '{"main":{"insertVml":"","updateVml":"","queryVml":"tms_0027"},"OrderContains":{"insertVml":"","updateVml":"","queryVml":"tms_0201","deleteVml":""}}';
</script>
 <table class="FormTable Y_alter line_chaidan" style="width:100%;" id="25ccf24c-9c73-44a5-899b-2565511d81d0" path="Price/eyxhzqmm" code="eyxhzqmm">
  ...
  <tbody>
    <tr>
        <td class="text-left" rowspan="1" colspan="5" style="font-size: 14px; border: 0; border-bottom: 1px solid #e1e6eb; padding-right: 0; padding-left: 0;">
        <div style="padding-left: 3px; background-color: #f27302;">
            <p style="background-color: white; padding-left: 8px; height: 20px; line-height: 20px; margin-bottom: 25px;">市内合单信息</p>
        </div>
        </td>
      <td colspan="6" style="border:0;">
        ...
        <div class="toolbar">
          <div style="text-align:right;" class="button_workflow">
            <a class="btn btn-red footKeepBtn" href="javascript:void(0);" onclick="CombineOrders(1,1)">发送&nbsp;<span class="glyphicon glyphicon-send"></span></a>
            <a class="btn btn-red footKeepBtn" href="javascript:void(0);" onclick="CombineOrders(0,1)">保存为草稿&nbsp;<span class="glyphicon glyphicon-floppy-disk"></span></a>
    ...
    <tr style="height: 26px;">
      ...
      <td class="" rowspan="1" colspan="4">
         <input name="PactCode" class="edit form-control transparent"  f-options="{'code':'PactCode','type':'text','etype':'editable','len':'100'}" verify="{'title':'合同编号','length':'100','required':true}">
            ...
            <input type="hidden" name="ShipMode" value="1">
  ...
  <tr style="height: 26px;">
    ...
    <tr style="height: 26px;">
      ...
      <td class="backgroundGray" rowspan="1" colspan="">收货方名称</td>
    ...
    <tr style="height: 26px; display: none;"  class="" fb-options="{'rowid':'MinPrice','initialRows':'','maxrows':'','content':''}" rowid="OrderContains">
      ...
      <td class="" rowspan="1" colspan="">
        <input name="EndUserName" class="edit form-control transparent" readonly f-options="{'code':'EndUserName','type':'number','etype':'','len':''}" verify="{}">
      </td>
      <td class="" rowspan="1" colspan="">
        <input name="IFrom" class="edit form-control transparent" readonly f-options="{'code':'IFrom','type':'number','etype':'','len':''}" verify="{}">
      </td>
      <td class="" rowspan="1" colspan="">
        <input name="Fromtime" readonly class="edit form-control transparent" f-options="{'code':'Fromtime','type':'date','etype':'','len':''}" verify="{}">
      </td>
      <td class="" rowspan="1" colspan="">
        <input name="ITo" class="edit form-control transparent" readonly f-options="{'code':'ITo','type':'number','etype':'','len':''}" verify="{}">
      </td>
      <td class="" rowspan="1" colspan="">
        <input name="Totime" readonly class="edit form-control transparent" f-options="{'code':'Totime','type':'date','etype':'','len':''}" verify="{}">
      </td>
      <td class="" rowspan="1" colspan="">
        <input name="weight" class="edit form-control transparent" readonly f-options="{'code':'weight','type':'number','etype':'','len':''}" verify="{'title':'weight','length':'','textarea1':true}">
      </td>
      <td class="" rowspan="1" colspan="">
        <input name="volume" class="edit form-control transparent" readonly f-options="{'code':'volume','type':'number','etype':'','len':''}" verify="{'title':'volume','length':'','textarea1':true}">
      </td>
      <td class="" rowspan="1" colspan="">
        <input name="amount" class="edit form-control transparent" readonly f-options="{'code':'amount','type':'number','etype':'','len':''}" verify="{}">
      </td>
      <td class="">
        <a href="javascript:void(0);" tar="OrderContains" onclick="deleteOrder(this)" class="button edit" readonly executecode="%23delete%23"  f-options="{'code':'','type':'_button','etype':'','len':''}" verify="{'title':'','length':'','textarea1':true}">删除</a>
      </td>
    </tr>
...
<script type="text/javascript">
    var reqeustDone = function () {   /*所有JS加载完成以后执行*/
        ...
        initTableForm($('#25ccf24c-9c73-44a5-899b-2565511d81d0'),'',function() {
          setTimeout("hoverTips();totalOrderData();", 300);
        });
    };
