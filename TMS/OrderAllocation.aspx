...
<html>
<head>		
<title>运输计划-<%=NDS.Kits.Config.Instance.WebsiteParameters("ProductName")%></title>
<meta charset="utf-8" />
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer" style="position:absolute; top:0; width:100%; padding-bottom:0;">
					<button type="button" class="btn btn-default  okButton footKeepBtn" style="margin-right:8px; box-shadow:none;text-shadow:none;" aria-label="OK" title="确定" onclick="ClosedOrder()" ><span aria-hidden="true" class="">确定</span><span class="glyphicon glyphicon-ok" style="margin-left:2px;"></span></button>
		...
		<table class="jptable table" id="demoList">
			...
			{{each data}}
				<tr>
					...
					<td class="title" title="{{$value.code}}" fld="code"><a target="_blank" href="OrderAllocation_edit.aspx?id={{$value.id}}" >{{$value.code}}</a></td>
                    ...
                    <td class="title">
						<a href="javascript:void(0)"　title="提交订单" onclick="SplitSingleList({{$value.id}})" class="mainColor">提交</a>
                        <a href="javascript:void(0)"　title="关闭订单" onclick="Closed({{$value.id}})">关闭</a>						
					</td>
    			</tr>
			{{/each}}
			</tbody>
...
<script type="text/javascript">
    ...
    var reqeustDone = function () {		/*所有JS加载完成以后执行*/
        ...
        InitList($("#demo"), $("#demo .list"), $("#jplist-template").html(), "OrderAllocation", "/Widget.aspx?param=jplist&vid=grid_jplist_0007");
