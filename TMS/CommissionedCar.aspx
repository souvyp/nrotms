...
<html>
<head>		
<title>已委托拼车单-<%=NDS.Kits.Config.Instance.WebsiteParameters("ProductName")%></title>
<meta charset="utf-8" />
		...
		<table class="jptable table">
			...
			{{each data}}
				<tr>
					...
					<td class="title"  fld="Code" title="{{$value.Code}}"><a target="_blank" href="CommissionedCar_edit.aspx?id={{$value.id}}&SupplierID={{$value.SupplierID}}" >{{$value.Code}}</a></td>
...
<script type="text/javascript">
    var reqeustDone = function () {		/*所有JS加载完成以后执行*/
        ...
        InitList($("#demo"), $("#demo .list"), $("#jplist-template").html(), "CommissionedCar", "/Widget.aspx?param=jplist&vid=grid_jplist_0070");
