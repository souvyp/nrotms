...
<html>
<head>		
<title>拼车待调度-<%=NDS.Kits.Config.Instance.WebsiteParameters("ProductName")%></title>
<meta charset="utf-8" />
                ...
                <div class="modal-body">
                    <div class="row">
                        ...
                        <div class="col-md-10">
                            ...
                            <input type="hidden" name="PactCode" />
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header" style=" padding-bottom:0;padding-top:20px;">
                    ...
                    <h4 class="modal-title text-left" id="H1">
                        <div style="padding-left: 3px; background-color: #f27302;">
                           <p style="background-color: white; padding-left: 8px; height: 20px;margin-bottom:0; line-height: 20px; margin-bottom: 20px;color:#666; font-size:14px;font-family:'微软雅黑';">关闭拼车单</p>
                        </div>
                    </h4>
                </div>
                <div class="modal-body">
                ...
                <div class="modal-footer" style="position:absolute; top:0; width:100%; padding-bottom:0;">
					<button type="button" class="btn btn-default  okButton" style="margin-right:8px; box-shadow:none;" aria-label="OK" title="确定" onclick="CloseCombineOrder( $(this).closest('div#ClosedModal'))" ><span aria-hidden="true" class="">确定</span><span class="glyphicon glyphicon-ok" style="margin-left:2px;color:#888;"></span></button>
					<button type="button" class="btn btn-default " data-dismiss="modal" aria-label="Close" title="关闭" style="box-shadow:none;margin-left:0;"><span aria-hidden="true" class="">关闭</span><span class="glyphicon glyphicon-remove" style="margin-left:2px;color:#888;"></span></button>
                </div>
            </div>
            <!-- /.modal-content -->
	...
	<div class=""><!-- block right -->
		<table class="jptable table" id="demoList">
			...
			{{each data}}
				<tr>
					<td style="width:40px">
                    <input type="hidden" name="SupplierID" fld="SupplierID" value="{{$value.SupplierID}}"/>
                    <input type="hidden" name="id" fld="id" value="{{$value.id}}"/>{{$index+1}}</td>
					<td class="title"  fld="Code" title="{{$value.Code}}"><a  href="CombinedSend_edit.aspx?id={{$value.id}}&SupplierID={{$value.SupplierID}}" >{{$value.Code}}</a></td>
					<td class="title" title="{{$value.PactCode}}" fld="PactCode">{{$value.PactCode}}</td>
                    <td class="title" title="{{$value.SupplierName}}" fld="SupplierName">
                    ...
                    <td class="title">
                    	<button class="btn btn-link" title="发送拼车单" onclick="CombinedModal({{$value.id}}, '{{$value.SupplierName}}', '{{$value.OpenId}}', '{{$value.Qty}}', '{{$value.PactCode}}')" style="margin-left:0;">发送</button>
...
<script type="text/javascript">
    ...
    var reqeustDone = function () {		/*所有JS加载完成以后执行*/
        ...
        InitList($("#demo"), $("#demo .list"), $("#jplist-template").html(), "OrderSendCar", "/Widget.aspx?param=jplist&vid=grid_jplist_0069");
