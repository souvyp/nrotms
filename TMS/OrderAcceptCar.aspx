...
<html>
<head>		
<title>待接收拼车单-<%=NDS.Kits.Config.Instance.WebsiteParameters("ProductName")%></title>
<meta charset="utf-8" />
				...
				<tr>
					<td style="width:35px">
                         ...
                         <span style="position:relative; top:-1px;display:inline-block;width:16px;">{{$index+1}}</span>
                          <input type="checkbox" class="receive-checkbox" value="{{$value.id}}" srcClass="3">
                    </td>
					<td class="title"  fld="Code" title="{{$value.Code}}"><a href="OrderAcceptCar_edit.aspx?id={{$value.id}}" >{{$value.Code}}</a></td>
					...
					<td class="title">
                        <button type="button"  title="接收订单" onclick = "IsReceiveCombineOrders( 1, {{$value.id}})" >接收</button>
...
<!--<><><><><><><><><><><><><><><><><><><><><><><><><><> 数据列表开始 <><><><><><><><><><><><><><><><><><><><><><><><><><>-->
		<div id="demo" class="box jplist">
			...
			<div class="jplist-panel box panel-top min_height">
                ...
                <div>
                    <button type="button" class="btn btn-default footKeepBtn" style="text-shadow:none;" onclick="lotsReceive()" title="批量接收" >批量接收&nbsp;<i class="glyphicon glyphicon-import"></i></button>
                </div>
...
<script type="text/javascript">
    var reqeustDone = function () {		/*所有JS加载完成以后执行*/
        ...
        InitList($("#demo"), $("#demo .list"), $("#jplist-template").html(), "OrderAcceptCar", "/Widget.aspx?param=jplist&vid=grid_jplist_0067", false, checkboxList);
