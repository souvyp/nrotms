...
<html>
<head>		
<title>已关闭拼车单-<%=NDS.Kits.Config.Instance.WebsiteParameters("ProductName")%></title>
<meta charset="utf-8" />
...
<div class="list-item box jplist-panel tabbtn">
	<div class=""><!-- block right -->
		<table class="jptable  table" id="demoList">
			...
			{{each data}}
				<tr>
					<td style="width:40px"><input type="hidden" name="SupplierID" fld="SupplierID" value="{{$value.SupplierID}}"/><input type="hidden" name="id" fld="id" value="{{$value.id}}"/>{{$index+1}}</td>
					<td class="title"  fld="Code" title="{{$value.Code}}"><a target="_blank" href="ClosedCommis_edit.aspx?id={{$value.id}}" >{{$value.Code}}</a></td>
...
<script type="text/javascript">
    ...
    var reqeustDone = function () {		/*所有JS加载完成以后执行*/
        ...
        InitList($("#demo"), $("#demo .list"), $("#jplist-template").html(), "ClosedManage", "/Widget.aspx?param=jplist&vid=grid_jplist_0089");
