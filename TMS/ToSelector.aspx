...
<html>
<head>		
<title>常用收货地址选择-<%=NDS.Kits.Config.Instance.WebsiteParameters("ProductName")%></title>
<meta charset="utf-8" />
...
<body code="ToSelector" style="width:800px">
<!-- 通用对话框开始-->
		...
		<table class="jptable table">
			...
			{{each data}}
				<tr onDblClick="dblEvents()">
					...
					<td class="title" fld="AddrName">{{if $value.ProvinceName != "" && $value.CityName != "" && $value.DistrictName != "" }}{{$value.ProvinceName}}-{{$value.CityName}}-{{$value.DistrictName}}{{else if $value.ProvinceName != "" && $value.CityName != "" }}{{$value.ProvinceName}}-{{$value.CityName}}{{/if}}
					</td>
					...
					<td style="display:none;" class="title" fld="ToName">{{$value.ToName}}</td>
				</tr>
			{{/each}}
			</tbody>
		</table>
...
<!--<><><><><><><><><><><><><><><><><><><><><><><><><><> 数据列表开始 <><><><><><><><><><><><><><><><><><><><><><><><><><>-->
		<div id="demo" class="box jplist">
			...
			<div class="jplist-panel box panel-top min_height">
			    ...
			    <div class="text-filter-box"><input data-path="Address" data-button="#desc-search-button" type="text" value="" placeholder="地址" data-control-type="textbox" data-control-name="Address" data-control-action="filter"> <button type="button" id="desc-search-button"><i class="glyphicon glyphicon-search"></i></button></div>
                <div class="text-filter-box hidden">
					<select data-control-type="filter-select" data-control-name="EndUserID" data-control-action="filter">
...
<script type="text/javascript">
    var reqeustDone = function () {		/*所有JS加载完成以后执行*/
        ...
        InitList($("#demo"), $("#demo .list"), $("#jplist-template").html(), "ToSelector", "/Widget.aspx?param=jplist&vid=grid_jplist_0023");
    };
