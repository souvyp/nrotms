...
<html>
<head>		
<title>常用地址-<%=NDS.Kits.Config.Instance.WebsiteParameters("ProductName")%></title>
<meta charset="utf-8" />
		...
		<table class="jptable table">
			...
			{{each data}}
				<tr>
					...
					<td class="title" fld="CustomerName"><a  href="CustomerAddrDetail_edit.aspx?id={{$value.id}}">{{$value.CustomerName}}</a></td>
					...
					<td class="title">
                        ...
                        {{else}}
                        <button type="button"  title="启用"   onclick="EODCustomerInfo('3', '1','{{$value.id}}')" style="margin-left:0;">启用</button>
                        {{/if}}

                        {{if $value.Invalid == "1"}}
                        <span readonly style="margin:0 5px;">禁用</span>
                        {{else}}
                        <button type="button"  title="禁用" onclick="EODCustomerInfo('3', '0','{{$value.id}}')" >禁用</button>
                        {{/if}}   
                    </td>				
                </tr>			{{/each}}
			</tbody>
...
<!--<><><><><><><><><><><><><><><><><><><><><><><><><><> 数据列表开始 <><><><><><><><><><><><><><><><><><><><><><><><><><>-->
		<div id="demo" class="box jplist">
            ...
            <div class="maintitle_container">
               <p class="mainHtml_tit">常用地址</p>
            </div>
			<!-- 操作面板 -->
			<div class="jplist-panel box panel-top min_height">
				<button type="button" class="form_sub" onclick="javascript:location.href='CustomerAddr_edit.aspx'">新增&nbsp;<i class="glyphicon glyphicon-plus-sign"></i></button>
			...
			<div class="text-filter-box" style="display:none;"><input data-path="opt_status" type="text" value="0" data-button="#opt_status-search-button" data-control-type="textbox" data-control-name="opt_status" data-control-action="filter"/><button type="button" id="opt_status-search-button"></button></div>
				<div class="text-filter-box"><!--[if lt IE 10]><div class="jplist-label">姓名:</div><![endif]--><input data-path="CustomerName" data-button="#name-search-button" type="text" value="" placeholder="客户名称" data-control-type="textbox" data-control-name="CustomerName" data-control-action="filter"> <button type="button" id="name-search-button"><i class="glyphicon glyphicon-search"></i></button></div>
				<%--<!-- 分页结果 -->
...
<script type="text/javascript">
    var reqeustDone = function () {		/*所有JS加载完成以后执行*/
        ...
        InitList($("#demo"), $("#demo .list"), $("#jplist-template").html(), "CustomerAddr", "/Widget.aspx?param=jplist&vid=grid_jplist_0010");
    };
