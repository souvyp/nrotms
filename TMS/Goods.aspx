...
<html>
<head>		
<title>客户物品-<%=NDS.Kits.Config.Instance.WebsiteParameters("ProductName")%></title>
<meta charset="utf-8" />
		...
		<table class="jptable table" id="demoList">
			<thead>
				<tr class="trtitle">
                    ...
                    <td class="title">物品编码</td>
					<td class="title">物品名称</td>
					<td class="title">物品类型</td>
			...
			{{each data}}
				<tr>
					...
					<td class="title" fld="Code"><a  href="goodsDetail_edit.aspx?id={{$value.id}}">{{$value.Code}}</a></td>
                    <td class="title" fld="Name">{{$value.Name}}</td>
					<td class="title" fld="TypeName">{{$value.TypeName}}</td>
					...
					<td class="title">
                        ...
                        {{else}}
                        <button type="button"  title="启用"   onclick = "EODCustomerInfo('2', '1', '{{$value.id}}')"  style="margin-left:0;">启用</button>
                        {{/if}}

                        {{if $value.Invalid == "1"}}
                        <span readonly style="margin:0 5px;">禁用</span>
                        {{else}}
                        <button type="button"  title="禁用" onclick = "EODCustomerInfo('2', '0', '{{$value.id}}')"   >禁用</button>
                        {{/if}}   
                    </td>				
                </tr>			{{/each}}
			</tbody>
...
<!--<><><><><><><><><><><><><><><><><><><><><><><><><><> 数据列表开始 <><><><><><><><><><><><><><><><><><><><><><><><><><>-->
		<div id="demo" class="box jplist">
            ...
            <div class="maintitle_container">
               <p class="mainHtml_tit">客户物品</p>
            </div>
			<!-- 操作面板 -->
			<div class="jplist-panel box panel-top min_height">
				<button type="button" class="form_sub" onclick="javascript:location.href='goods_edit.aspx'">新增&nbsp;<i class="glyphicon glyphicon-plus-sign"></i></button>
...
<script type="text/javascript">
    ...
    var reqeustDone = function () {		/*所有JS加载完成以后执行*/
        ...
        InitList($("#demo"), $("#demo .list"), $("#jplist-template").html(), "goods", "/Widget.aspx?param=jplist&vid=grid_jplist_0005");
