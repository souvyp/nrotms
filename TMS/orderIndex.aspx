...
<link href="<%=MinifyUrl("ListCss")%>" rel="stylesheet" type="text/css" />
 <style>
     ...
     .searchList .text-filter-box.active {
         display:block;
     }
     .searchList .text-filter-box{
         display:none;
     }
     .jplist-panel .jplist-drop-down li:hover{
     	background: #f27302;
     	color: white;
     }
     .jplist-drop-down{
			...
			{{each data}}
				<tr>
					...
					<td class="title" title="{{$value.code}}" fld="code">
						{{if $value.status > '2'}}
							<a target="_blank" href="Index_edit.aspx?id={{$value.id}}">{{$value.code}}</a>
						{{else}}
							<a target="_blank" href="Indextrans_edit.aspx?id={{$value.id}}&code=orderIndex">{{$value.code}}</a>
						{{/if}}
					</td>
		...
		<div id="demo" class="box jplist">
			...
			<div class="jplist-panel box panel-top min_height">
			   ...
			   <div class="text-filter-box" style="display:none;">
                ...
                <div class="text-filter-box"><!--[if lt IE 10]>
					<div class="jplist-label">制单时间:</div><![endif]-->
...
<script type="text/javascript">
    ...
    var reqeustDone = function () {		/*所有JS加载完成以后执行*/
        ...
        InitList($("#demo"), $("#demo .list"), $("#jplist-template").html(), "Index", "/Widget.aspx?param=jplist&vid=grid_jplist_0029");
