       ...
       <div class="modal-dialog" style="height:170px">
          <div class="modal-content" style="height:100%;width:64%">
                ...
                <div class="modal-footer" style="position:absolute; top:0; width:100%; padding-bottom:0;">
					<button type="button" class="btn btn-default  okButton footKeepBtn" style="margin-right:8px; box-shadow:none;text-shadow:none;" aria-label="OK" title="确定" onclick="skiptoprint($(this).closest('#printTBModal'), 5)" ><span aria-hidden="true" class="">确定</span><span class="glyphicon glyphicon-ok" style="margin-left:2px;"></span></button>
		...
		<table class="jptable  table" id="demoList">
			...
			{{each data}}
				<tr>
					...
					<td class="title" title="{{$value.code}}" fld="code"><a target="_blank" href="OrderPrice_edit.aspx?id={{$value.id}}&code=orderIndex">{{$value.code}}</a></td>
					...
					<td class="title" title="{{$value.total}}" fld="to">{{$value.total}}</td>
					...
					<td class="title" title="{{$value.ContainsOrderCode}}" fld="ContainsOrderCode"><a target="_blank" href="OrderAcceptedCar_edit.aspx?id={{$value.OwnerOrderID}}">{{$value.ContainsOrderCode}}</a></td>
				</tr>
			{{/each}}
			</tbody>
		</table>
...
<!--<><><><><><><><><><><><><><><><><><><><><><><><><><> 数据列表开始 <><><><><><><><><><><><><><><><><><><><><><><><><><>-->
		<div id="demo" class="box jplist">
			...
			<div class="jplist-panel box panel-top min_height">
				...
				<button class="btn btn-default navbar-left footKeepBtn" data-toggle="modal" onclick="OrderTrackingToXml(2)" title="导出">导出</button>
...
<script type="text/javascript">
    ...
    {		/*所有JS加载完成以后执行*/
        ...
        InitList($("#demo"), $("#demo .list"), $("#jplist-template").html(), "OrderTracking", "/Widget.aspx?param=jplist&vid=grid_jplist_0028");
