		...
		<table class="jptable table table-border">
			...
			{{each data}}
				<tr>
	                ...
	                <td class="title" fld="name" style="text-align:left;"  title="{{$value.name}}"><a  href="OrderMSymbol_edit.aspx?id={{$value.id}}">{{$value.name}}</a></td>
                   ...
                   <td class="title">
                        ...
                        {{else}}
                        <button type="button"  title="启用"    onclick =  DCMOSymbol('1','{{$value.id}}','4')   style="margin-left:0;">启用</button>
                        {{/if}}

                        {{if $value.invalid == "1"}}
                        <span readonly style="margin:0 5px;">禁用</span>
                        {{else}}
                        <button type="button"  title="禁用"   onclick =  DCMOSymbol('0','{{$value.id}}','4')  >禁用</button>
                        {{/if}} 
                    </td>		
				</tr>
			{{/each}}
...
<!--<><><><><><><><><><><><><><><><><><><><><><><><><><> 数据列表开始 <><><><><><><><><><><><><><><><><><><><><><><><><><>-->
		<div id="demo" class="box jplist">
			...
			<div class="jplist-panel box panel-top min_height">
				<button type="button" class="form_sub" onclick="javascript:location.href='OrderMSymbol_edit.aspx'">新增&nbsp;<i class="glyphicon glyphicon-plus-sign"></i></button>
...
<script type="text/javascript">
    var reqeustDone = function () {		/*所有JS加载完成以后执行*/
        ...
        InitList($("#demo"), $("#demo .list"), $("#jplist-template").html(), "MSymbol", "/Widget.aspx?param=jplist&vid=grid_jplist_0082");
