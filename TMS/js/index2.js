...
$('.add_place').click(function() {
    ...
    var addtr = "<tr nrowid='split'><td style='border-right:1px solid #e1e6eb;'><span style='float:left;color:#666;'>" +
        "<a href=\"javascript:void(0);\" onclick=\"showModalWindow(this,\'收货方\')\" class=\"edit shr\" f-options=\"{'code':'EndUserID','type':'dialogue','etype':'editable','len':'50','cls':'frame','url':'PlayEndUserSelector.aspx','vals':'EndUserName=name','modalWindow':'1'}\" verify=\"{}\"><input type=\"hidden\" name=\"EndUserID\">收货人：</a></span></td>" +
        "<td><input style='display:inline-block;width:80px !important;' name=\"EndUserName\" placeholder=\"点击“收货人”进行选择或手动书写\" title=\"点击“收货人”进行选择或手动书写\" class=\"edit form-control transparent writeInput\" f-options=\"{'code':'EndUserName','type':'text','etype':'editable','len':'50'}\" verify=\"{}\"></td>" +
        "<td colspan='1' style='border-left:1px solid #e1e6eb;'><span style='float:left;color:#666;'>" +
        "<a style='display:inline-block;width:60px;' href=\"javascript:void(0);\" onclick=\"showModalWindow(this,'途经地',$(this).closest('tr').find('input[name=EndUserID]' ))\" class=\"edit\" f-options=\"{'code':'id','type':'dialogue','etype':'editable','len':'32','cls':'frame','url':'ToSelector.aspx','vals':'InputDedail=Address,FromProvince=ProvinceID,FromCity=CityID,FromDistrict=DistrictID,WayTo=AddrName,WayToPhone=Phone','modalWindow':'1'}\" verify=\"{}\">途经地：</a></span></td><td colspan='4'><input type='text' name='WayTo' placeholder='请输入省市区或点击“途经地”选择' readonly  class='city_input inputFocus proCitySelAll' style='border:0;width:180px;line-height:34px;'/>" +
        ...
        "</td><td colspan='3' style='border: 1px solid #e1e6eb;'><input type='text' name='InputDedail' placeholder='请输入详细地址' class='city_input inputFocus' style='border:0;width:140px;line-height:34px;'/></td><td style='width:60px;padding-right:0;border-left: 1px solid #e1e6eb;'>联系电话</td><td><input type=\"text\"  name=\"WayToPhone\" placeholder=\"联系电话\" style=\"border: 0; width: 80px; line-height: 34px;\" /></td><td  style=\"border: 1px solid #e1e6eb;\"><span style=\"float: left; color: #666;\">";
    if (navigator.userAgent.indexOf('Firefox') >= 0)
        addtr += "时间：</span></td><td class=\"\" ><input name=\"WayToTime\" readonly oc=\"date\" class=\"laydate-icon edit\" onclick=\"laydate( { format: 'YYYY/MM/DD', min: '" + startTime + "' } )\" f-options=\"{'code':'ToTime','type':'date','etype':'editable','len':'50'}\" verify=\"{}\"></td><td  style='border:1px solid #e1e6eb;'><a class='delete_tr pull-right' style=' margin-left:10px;'>删除</a></td></tr>";
    else
        addtr += "时间：</span></td><td class=\"\" ><input name=\"WayToTime\" readonly oc=\"date\" class=\"laydate-icon edit\" onclick=\"TrigerDateEvent( this, { format: 'YYYY/MM/DD' ,min:'" + startTime + "'} )\" f-options=\"{'code':'ToTime','type':'date','etype':'editable','len':'50'}\" verify=\"{}\"></td><td  style='border:1px solid #e1e6eb;'><a class='delete_tr pull-right' style=' margin-left:10px;'>删除</a></td></tr>";
    if (startTime == "") {
        alert('请输入时间！');
    } else {
...
function SplitSingle() {
    ...
    var _fcnt = $('input[name="InputFromPhone"]').val();
    ...
    var _tcnt = $('input[name="InputToPhone"]').val();
    ...
    if (typeof($('input[name="WayTo"]').val()) != 'undefined') {
        //fp1,fc1,fd1,from1,tp1,tc1,td1,to1,time1,eu1,fcnt1,tcnt1|
        $('input[name="WayTo"]').each(function(index) {
            ...
            _to = _to.replaceAll( ",|，", "&#44;");
            AddrLst += '' + _fp + ',' + _fc + ',' + _fd + ',' + _from + ',' + _tp + ',' + _tc + ',' + _td + ',' + _to + ',' + _time + ',' + _eu + ',' + _fcnt + ',' + _tcnt;
        ...
        _tcnt = $('input[name="InputToPhone"]').val();
        _time = '';
        _eu = '';
    }
    _from = _from.replaceAll( ",|，", "&#44;");
    ...
    GoodsLst = GoodsLst.replace(/\d+=0,/g, '').replace(/,\d+=0/g, '');
...
function Schedule(){
    ...
    if( _readioval != undefined )
    {
        ...
        if(statTF)
        {
            ...
            _paras[6].value = _SupplierSymbolID;
            pmls[0].paras = _paras;
...
function skiptoprint(divObj, number){   
    ...
    if(number==1){
        ...
        window.open("OrderTrackingOutput.aspx?date="+_date, '_blank');
    }else if(number==2){
        window.open("TrackingManagePrint.aspx?date="+_date, '_blank');
    }else if(number==4){
        window.open("TrackingManageOutput.aspx?date="+_date, '_blank');
    }
...
$(function() {
    if (navigator.userAgent.indexOf('Firefox') >= 0) {
        ...
        $('input[name="PurchaseTime"], input.report-date, input[name="Insurance"],input[name="StartTime"],input[name="EndTime"],input[name="WayToTime"],input[placeholder="制单时间"],input[placeholder="起始时间"],input[placeholder="发货时间"],input[placeholder="结束时间"],input[name="Birthday"],input[name="LicensedDate"]').attr('onclick', 'laydate({ format: "YYYY/MM/DD hh:mm" })');
		$('#laydate_hms').show();	
...
{
	$('.current2,input[name="ToProvince"],input[name="ToCity"],input[name="ToDistrict"]').val('');
	$('#ToButton').click();
});
...
//拆分物品,显示模态框
function splitQtyClick( Qty, ListID ){
    $('div#splitQtyModal').modal( "show" );
    $('div#splitQtyModal span[name="totalQty"]').text( Qty );
    $('div#splitQtyModal span[name="ListID"]').text( ListID );
    $('input.qtyinput').val("");
}
//拆分物品,模态框确认
