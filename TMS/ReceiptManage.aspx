...
<html>
<head>		
<title>回单管理-<%=NDS.Kits.Config.Instance.WebsiteParameters("ProductName")%></title>
<meta charset="utf-8" />
...
<!--输入备注信息开始-->
    <div class="modal fade" id="AppendComModal" tabindex="-1" role="dialog"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header" style=" padding-bottom:0;padding-top:20px;">
                    <h4 class="modal-title text-left" id="H1">
                        <div style="padding-left: 3px; background-color: #f27302;">
                            <p style="background-color: white; padding-left: 8px; height: 20px;margin-bottom:0; line-height: 20px; margin-bottom: 20px;color:#666; font-size:14px;font-family:'微软雅黑';">追加备注</p>
                        </div>
                    </h4>
                </div>
                <div class="modal-body">
                    <div class="row">
                        <label class="col-md-2 control-Label">追加备注</label>
                        <div class="col-md-10">
                ...
                <div class="modal-footer" style="position:absolute; top:0; width:100%; padding-bottom:0;">
                    <button type="button" class="btn btn-default  okButton footKeepBtn" style="margin-right:8px; box-shadow:none;" aria-label="OK" title="确定" onclick="AppendComments('#AppendComModal')" ><span aria-hidden="true" class="">确定</span><span class="glyphicon glyphicon-ok" style="margin-left:2px;"></span></button>
            ...
            <div class="modal-content">
                <div class="modal-header" style=" padding-bottom:0;padding-top:20px;">
                    ...
                    <h4 class="modal-title text-left" id="myModalLabel">
                        <div style="padding-left: 3px; background-color: #f27302;">
                            <p style="background-color: white; padding-left: 8px; height: 20px;margin-bottom:0; line-height: 20px; margin-bottom: 20px;color:#666; font-size:14px;font-family:'微软雅黑';">打回订单</p>
                        </div>
                    </h4>
                </div>
                <div class="modal-body">
                ...
                <div class="modal-footer" style="position:absolute; top:0; width:100%; padding-bottom:0;">
					<button type="button" class="btn btn-default  okButton" style="margin-right:8px; box-shadow:none;" aria-label="OK" title="确定" onclick="SendBackOrder()" ><span aria-hidden="true" class="">确定</span><span class="glyphicon glyphicon-ok" style="margin-left:2px;color:#888;"></span></button>
    ...
    <div class="modal fade" id="ClosedModal" tabindex="-1" role="dialog"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer" style="position:absolute; top:0; width:100%; padding-bottom:0;">
					<button type="button" class="btn btn-default  okButton" style="margin-right:8px; box-shadow:none;" aria-label="OK" title="确定" onclick="ClosedOrder()" ><span aria-hidden="true" class="">确定</span><span class="glyphicon glyphicon-ok" style="margin-left:2px;color:#888;"></span></button>
					<button type="button" class="btn btn-default " data-dismiss="modal" aria-label="Close" title="关闭" style="box-shadow:none;margin-left:0;"><span aria-hidden="true" class="">关闭</span><span class="glyphicon glyphicon-remove-sign" style="margin-left:2px;color:#888;"></span></button>
                </div>
            </div>
            <!-- /.modal-content -->
        </div>
   ...
   aria-labelledby="myModalLabel" aria-hidden="true">
       <div class="modal-dialog" style="height:200px">
          <div class="modal-content" style="height:100%">
                ...
                <h4 class="modal-title text-left" id="H3">
                    <div style="padding-left: 3px; background-color: #f27302;">
		...
		<table class="jptable table" id="demoList">
			...
			{{each data}}
				<tr>
					...
					<td class="title" title="{{$value.code}}" fld="code"><a  href="ReceiptManage_edit.aspx?id={{$value.id}}">{{$value.code}}</a></td>
					...
					<td class="title" title="{{$value.to}}" fld="to">{{$value.to}}</td>
                     <td class="title" title="{{$value.createtime}}" fld="createtime">{{$value.createtime}}</td>
                    <td class="title" title="{{$value.SupplierName}}" fld="SupplierName">{{$value.SupplierName}}</td>
                    <td class="title">
						<button class="btn btn-link" title="提交纸质回单" onclick="IsHd({{$value.id}})" style="color:#f27302;margin-left:0;">回单</button>
						<span class="hide receiptdoc">{{$value.receiptdoc}}</span>
						<span class="hide receiptDoc1">{{$value.receiptDoc1}}</span>
						<span class="hide receiptDoc2">{{$value.receiptDoc2}}</span>
                        <button type="button" title="追加备注" onclick="addComment({{$value.id}})" style="margin-right:5px;">备注</button>
                        <!--{{ if $value.combined == "0" }}
...
<!--<><><><><><><><><><><><><><><><><><><><><><><><><><> 数据列表开始 <><><><><><><><><><><><><><><><><><><><><><><><><><>-->
		<div id="demo" class="box jplist">
			...
			<div class="jplist-panel box panel-top min_height">
				...
				<div class="text-filter-box">
                    <input data-path="rangefilter_createtime" class="rangeModal" data-button="#search-button"  type="text" value="" placeholder="制单时段" data-control-type="textbox" data-control-name="rangefilter_createtime" data-control-action="filter">
				</div>
                <div class="text-filter-box">
                    <!--<input readonly="" data-path="FromTime" onclick="GetDateEvent(this, { format: 'YYYY-MM-DD' })" data-button="#search-button" type="text" value="" placeholder="发货时间" data-control-type="textbox" data-control-name="FromTime" data-control-action="filter">--> 
				...
				<div class="collapse clearfix" id="collapseExample" style="margin-left:238px;">
                    ...
                    <div class="text-filter-box">
                        <input data-path="SupplierName" data-button="#opt_status-search-button" type="text" value="" placeholder="承运方名称" data-control-type="textbox" data-control-name="SupplierName" data-control-action="filter" />
                    </div>
				</div>			
...
<script type="text/javascript">
    ...
    var reqeustDone = function () {		/*所有JS加载完成以后执行*/
        ...
        InitList($("#demo"), $("#demo .list"), $("#jplist-template").html(), "ReceiptManage", "/Widget.aspx?param=jplist&vid=grid_jplist_0040");
