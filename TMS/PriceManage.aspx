...
<html>
<head>		
<title>订单费用-<%=NDS.Kits.Config.Instance.WebsiteParameters("ProductName")%></title>
<meta charset="utf-8" />
       ...
       <div class="modal-dialog" style="height:170px">
          <div class="modal-content" style="height:100%;width:64%">
                ...
                <div class="modal-footer" style="position:absolute; top:0; width:100%; padding-bottom:0;">
					<button type="button" class="btn btn-default  okButton footKeepBtn" style="margin-right:8px; box-shadow:none;text-shadow:none;" aria-label="OK" title="确定" onclick="skiptoprint($(this).closest('#printTBModal'), 6)" ><span aria-hidden="true" class="">确定</span><span class="glyphicon glyphicon-ok" style="margin-left:2px;"></span></button>
		...
		<table class="jptable  table" id="demoList">
			...
			{{each data}}
				<tr>
					...
					<td class="title" title="{{$value.code}}" fld="code"><a target="_blank" href="PriceManage_edit.aspx?id={{$value.id}}&code=TransOrder">{{$value.code}}</a></td>
					...
					<td class="title" title="{{$value.to_address}}" fld="to_address">{{$value.to_address}}</td>
					<td class="title" title="{{$value.total}}" fld="total">{{$value.total}}</td>
					...
					<td class="title" title="{{$value.ContainsOrderCode}}" fld="ContainsOrderCode">
						{{if $value.status == '1'}}
						...
						{{else}}
							<a target="_blank" href="CommissionedCar_edit.aspx?id={{$value.OwnerOrderID}}">{{$value.ContainsOrderCode}}</a>
						{{/if}}
					</td>
				</tr>
			{{/each}}
		...
		<div id="demo" class="box jplist">
			...
			<div class="jplist-panel box panel-top min_height">
			   ...
			   <div class="text-filter-box" style="display:none;">
				...
				<div class="text-filter-box">
                    <input data-path="SupplierName" class="data" name="SupplierName" data-button="#opt_status-search-button" type="text" value="" placeholder="承运方名称" data-control-type="textbox" data-control-name="SupplierName" data-control-action="filter">
				...
				<button class="btn btn-default navbar-left footKeepBtn" data-toggle="modal" onclick="OrderTrackingToXml(4)" title="导出">导出</button>
...
<script type="text/javascript">
    ...
    {		/*所有JS加载完成以后执行*/
        ...
        InitList($("#demo"), $("#demo .list"), $("#jplist-template").html(), "TrackingManage", "/Widget.aspx?param=jplist&vid=grid_jplist_0087");
