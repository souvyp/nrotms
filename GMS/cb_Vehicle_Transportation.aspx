...
<html>
<head>		
<title>集团成本-整车·零担报表-<%=NDS.Kits.Config.Instance.WebsiteParameters("ProductName")%></title>
<meta charset="utf-8" />
...
<body code="cb_Vehicle_Transportation">  
<style type="text/css">
...
<!--<><><><><><><><><><><><><><><><><><><><><><><><><><> 数据列表开始 <><><><><><><><><><><><><><><><><><><><><><><><><><>-->
		<div id="demo" class="box jplist">
            ...
            <div class="maintitle_container">
               <p class="mainHtml_tit">集团成本-整车·零担报表</p>
            </div>
 						<button class="btn btn-default footKeepBtn Vehicle_b long_active" value="2" name="long">整车</button>
 						<button class="btn btn-default footKeepBtn Vehicle_b"value="1"  name="long">零担</button> 
 						<select class="year_" style="margin:0 12px;" onchange="Vehicle_Transportation_cb()">
...
<script type="text/javascript">
	$(document).on('click', '.Vehicle_b', function() {
			...
			function(result, data) {
				if(result) {
					...
					if(length != 0) {
						...
						ColumnChart('highcharts', year, year , chartsName, series, sasd);
					} else {
						$('.jplistYL').html("");
