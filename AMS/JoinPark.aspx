		...
		<table class="jptable table" id="demoList">
			...
			{{each data}}
				<tr>
                   ...
                   <td class="title">
                   		{{if $value.Status == "0"}}
	                        <button type="button"  title="发送"   onclick =  "SendParkReq({{$value.id}})" style="margin-left:0;">发送</button>
	                        <button type="button"  title="关闭"   onclick =  "CloseGroup({{$value.id}})">关闭</button>    
                   		{{/if}}

                   		{{if $value.Status == "2" && $value.Invalid == "0"}}
                   			<button type="button"  title="退出"   onclick =  "ZoneCompanyEOD({{$value.id}})">退出</button>    
                   		{{/if}}
                    </td>
    			</tr>
			{{/each}}
...
<!--<><><><><><><><><><><><><><><><><><><><><><><><><><> 数据列表开始 <><><><><><><><><><><><><><><><><><><><><><><><><><>-->
		<div id="demo" class="box jplist">
			...
			<div class="jplist-panel box panel-top min_height">
				<button type="button" class="form_sub" onclick="javascript:location.href='JoinPark_edit.aspx'">新增&nbsp;<i class="glyphicon glyphicon-plus-sign"></i></button>
...
<script type="text/javascript">
    ...
    var reqeustDone = function () {		/*所有JS加载完成以后执行*/
        ...
        InitList($("#demo"), $("#demo .list"), $("#jplist-template").html(), "JoinGroup", "/Widget.aspx?param=jplist&vid=grid_jplist_0108");
