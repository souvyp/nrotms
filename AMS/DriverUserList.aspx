...
<html>
<head>		
<title>个体司机用户-<%=NDS.Kits.Config.Instance.WebsiteParameters("ProductName")%></title>
<meta charset="utf-8" />
		...
		<table class="jptable table">
			...
			{{each data}}
				<tr>
					...
					<td class="title" fld="Name"><a  href="DriverUserList_edit.aspx?id={{$value.id}}" >{{$value.Name}}</a></td>
...
<script type="text/javascript">
    var reqeustDone = function () {		/*所有JS加载完成以后执行*/
        ...
        InitList($("#demo"), $("#demo .list"), $("#jplist-template").html(), "FMSUserList", "/Widget.aspx?param=jplist&vid=grid_jplist_0114",'', function() {
        	if ($('.jptable tbody tr').length > 0) {
              $('button.form_sub').addClass('hide');
            }
        });
    };
