...
<html>
<head>		
<title>集团审核申请信息-<%=NDS.Kits.Config.Instance.WebsiteParameters("ProductName")%></title>
<meta charset="utf-8" />
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header" style=" padding-bottom:0;padding-top:20px;">
                    ...
                    <h4 class="modal-title text-left" id="H1">
                        <div style="padding-left: 3px; background-color: #f27302;">
                            <p style="background-color: white; padding-left: 8px; height: 20px;margin-bottom:0; line-height: 20px; margin-bottom: 20px;color:#666; font-size:14px;font-family:'微软雅黑';">审核园区申请</p>
                        </div>
                    </h4>
                </div>
                <div class="modal-body">
                ...
                <div class="modal-footer" style="position:absolute; top:0; width:100%; padding-bottom:0;">
					<button type="button" class="btn btn-default  okButton footKeepBtn" style="margin-right:8px; box-shadow:none;" aria-label="OK" title="同意" onclick="ConfirmPark( 1, $(this).closest('div#ConfirmModal') )" ><span aria-hidden="true" class="">同意</span><span class="glyphicon glyphicon-ok" style="margin-left:2px;"></span></button>
					<button type="button" class="btn btn-default  okButton footKeepBtn" style="margin-right:8px; box-shadow:none;" aria-label="OK" title="拒绝" onclick="ConfirmPark( 0, $(this).closest('div#ConfirmModal') )" ><span aria-hidden="true" class="">拒绝</span><span class="glyphicon glyphicon-ban-circle" style="margin-left:2px;"></span></button>
		...
		<table class="jptable table" id="demoList">
			...
			{{each data}}
				<tr>
					...
					<td class="title" fld="CompanyName">
                        <a target="_blank" href="ConfirmPark_edit.aspx?id={{$value.id}}&status={{$value.Status}}">{{$value.CompanyName}}</a>
					</td>
                   ...
                   <td class="title">
                   		{{if $value.Status == "1" }}
                            <button type="button"  title="审核"   onclick =  "ConfirmGroup({{$value.id}})" >审核</button>
                        {{/if}}
...
<script type="text/javascript">
    ...
    var reqeustDone = function () {		/*所有JS加载完成以后执行*/
        ...
        InitList($("#demo"), $("#demo .list"), $("#jplist-template").html(), "ConfirmPark", "/Widget.aspx?param=jplist&vid=grid_jplist_0107");
