 ...
 <table class="FormTable Y_alter areaTable" style="width:100%;" id="25ccf24c-9c73-44a5-899b-2565511d81d0" path="Price/qksbgqsk" code="qksbgqsk">
  ...
  <tbody>
    <tr class="td-container tr-fixed">
       <td class="text-left" rowspan="1" colspan="5" style="padding-left:0;width:100% !important;">
        ...
        <div class="toolbar">
          <div style="text-align:right;" class="button_workflow">
            <%--<a class="btn btn-red" href="javascript:void(0);" onclick="printURLTable($('#25ccf24c-9c73-44a5-899b-2565511d81d0'))">打印&nbsp;<span class="glyphicon glyphicon-print"></span></a>--%>
              <a class="btn btn-red footKeepBtn saveBtn" href="javascript:void(0);" onclick="CompactSave(this)" style="display:none;">保存&nbsp;<span class="glyphicon glyphicon-floppy-disk"></span></a>
            <a class="btn btn-red footKeepBtn sendBtn" href="javascript:void(0);" title="发送报价单"  onclick="IsSend()" style="display:none;">发送&nbsp;<span class="glyphicon glyphicon-send"></span></a>
    ...
    <tbody id="PMS_LessLoad" role="tab" class="tab-pane active">
    	<tr class="insert">
    		<td colspan="10" style="border: 0px;border-bottom: 1px solid #ddd;padding-left:5px;padding-top:8px;">
    ...
    <tbody id="PMS_FullLoad" role="tab" class="tab-pane">
        <tr>
            <td class="backgroundGray" rowspan="1" colspan="3">起点省市</td>
            <td class="province setOff" rowspan="1" colspan="7">			    
                <input name="FromName" type="text" readonly placeholder="请选择起点省市" class="city_input inputFocus proCitySelAll"/>
        ...
        <tr style="height: 26px;">          
          <td class="backgroundGray" rowspan="1" colspan="3">目的省市</td>
        ...
        <tr style="height: 26px; display: none;" class="" fb-options="{'rowid':'PMS_FullLoad','initialRows':'','maxrows':'','content':''}" rowid="PMS_FullLoad">  
          <td class="province" rowspan="1" colspan="3">
              ...
              <input name="ToName" type="text" readonly placeholder="请选择目的省市" class="city_input inputFocus proCitySelAll"/>
                <input name="ToProvince" type="hidden" class="edit form-control transparent" f-options="{'code':'ToProvince','type':'text','etype':'','len':''}" verify="">
                <input name="ToCity" type="hidden" class="edit form-control transparent" f-options="{'code':'ToCity','type':'text','etype':'','len':''}" verify="">                 
                <div class="provinceCityAll">
                    <div class="tabs clearfix">
...
<script type="text/javascript">
    ...
    var reqeustDone = function () {		/*所有JS加载完成以后执行*/
        ...
        if (_sending) {
            ...
            $('input[name="From"],input[name="ToName"],input[name="FromName"]').attr('readonly','readonly');
            var linkA = '<a href="javascript:void(0);" onclick="showModalWindow(this,\'所属客户\')" class="edit" f-options="{\'code\':\'CustomerID\',\'type\':\'dialogue\',\'etype\':\'editable\',\'len\':\'50\',\'cls\':\'frame\',\'url\':\'CustomerSelector.aspx\',\'vals\':\'CustomerID=id,CustomerName=Name\',\'modalWindow\':\'1\'}" verify="{}"></a>';
            $('input[name="CustomerID"]').parent().wrap(linkA);
        }
        ...
        $( '#KG input, #Ton input, #CMetre input, #Square input, #num input' ).change( function (){
            ...
            {                
                ...
                $('li a[aria-controls="PMS_LessLoad"]').attr('style', 'color:#DF771E');
                if ( name == 'num' )
                {
            ...
            $('#' + name).find('tr').each(function (index) {
                if ($('#' + name).find('tr').eq(index).css('display') == "table-row") {
                        ...
                        if ($('#' + name).find('tr').eq(index).find('td').find('input[name="Price"]').val() != undefined && $('#' + name).find('tr').eq(index).find('td').find('input[name="Price"]').val() != '' && $('#' + name).find('tr').eq(index).find('td').find('input[name="Price"]').val() != '请选择目的城市' &&
                             parseInt($('#' + name).find('tr').eq(index).find('td').find('input[name="Price"]').val()) != 0) {
                            stat = true;
                        }
    ...
    function hasLD(tag) {
            ...
            var stat = false;
                ...
                if (stat == false) {
                    ...
                    if (name == 'num') {
                        $('li a[aria-controls="' + name + '"]').nextAll().find('button').attr('style', 'color:#DF771E');
                        $('li a[aria-controls="' + name + '"]').nextAll().find('button').find('span').css('color', '#DF771E');
                    }
                }
