			...
			{{each data}}
				<tr>
					...
					<td class="title" fld="CompanyName">
                        <a target="_blank" href="ConfirmGroupReq_edit.aspx?id={{$value.id}}&status={{$value.Status}}">{{$value.CompanyName}}</a>
					</td>
                    ...
                    <td class="title" title="{{$value.Invalid}}" fld="Invalid">
	                    {{if $value.Invalid == "0"}}
	                        有效
                   ...
                   <td class="title">
                        ...
                        {{else}}
	                        ...
	                        {{else}}
	                        	<button type="button"  title="启用"   onclick =  GroupCompanyEOD('1','{{$value.id}}') style="margin-left:0" >启用</button>
	                        {{/if}}

	                        {{if $value.Invalid=="1"}}
	                        	<span readonly style="margin:0 5px;">禁用</span>
	                        {{else}}
	                        	<button type="button"  title="禁用" onclick =  GroupCompanyEOD('0','{{$value.id}}') >禁用</button>
	                        {{/if}}
                        {{/if}}
...
<script type="text/javascript">
    ...
    var reqeustDone = function () {		/*所有JS加载完成以后执行*/
        ...
        InitList($("#demo"), $("#demo .list"), $("#jplist-template").html(), "ConfirmGroupReq", "/Widget.aspx?param=jplist&vid=grid_jplist_0096");
