...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_pub_price_EditOrderDoc]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[sp_pub_price_EditOrderDoc]
GO
...
/*创建或修改报价单*/
CREATE PROCEDURE [dbo].[sp_pub_price_EditOrderDoc]
(
	@Operator BIGINT,
	@DocID BIGINT = 0,
...
BEGIN
	...
	-- 合单报价
	IF @Result = 0 AND @Type IN (4,5) AND NOT EXISTS(SELECT Index_ID FROM TMS_OrderIndex WHERE Index_ID = @OrderID AND Index_Status <> 32 AND Index_Invalid = 0 AND Index_SrcClass = 3)
	BEGIN
...
Index_CustomerCompanyID = @CustomerCompanyID, Index_SupplierSymbolID = @SupplierSymbolID WHERE Index_ID = @DocID AND Index_Invalid = 0;
	...
	SELECT @Result AS Order_Result, @DocID AS Doc_ID;
