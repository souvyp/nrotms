...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_pub_order_ReceiveCombineDriverOrder]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[sp_pub_order_ReceiveCombineDriverOrder]
GO
...
/*接收或拒绝调度给我的拼车单*/
CREATE PROCEDURE [dbo].[sp_pub_order_ReceiveCombineDriverOrder]
(
	@OpenId NVARCHAR(300),
	@OrderID BIGINT,
