...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_pub_price_MinPrice]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[sp_pub_price_MinPrice]
GO
...
最低收费
*/
CREATE PROCEDURE [dbo].[sp_pub_price_MinPrice]
(
	@Operator BIGINT,
	@DocID BIGINT,				-- 报价单号
	@DetailID BIGINT = 0,       -- 价格编号
	@DocType BIGINT,			-- 报价类型，固定为7
...
BEGIN
	...
	IF @CompanyID <= 0
	BEGIN
		SET @Result = -5200081001;
	END
	
	-- 当前用户没有添加客户的权限
	IF @Result = 0 AND NOT EXISTS(SELECT [User_ID] FROM TMS_User WHERE [User_ID] = @Operator AND User_CompanyID = @CompanyID AND User_Invalid = 0)
	BEGIN
		SET @Result = -520008002;
	END
	
	-- 价格类型是否匹配(最低收费)
	IF @Result = 0 AND @DocType <> 7
	BEGIN
		SET @Result = -520008003;
	END
	IF @Result = 0 AND @Price <= 0
	BEGIN
		SET @Result = -520008007;
	END
...
Detail_Updater = @Operator, Detail_UpdateTime = GETDATE(), Detail_Invalid = 0 WHERE Detail_ID = @DetailID;
			IF @@ROWCOUNT = 0
			BEGIN
				SET @Result = -520008006;
			END
		END
	END
