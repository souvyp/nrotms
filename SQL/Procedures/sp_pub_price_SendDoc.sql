...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_pub_price_SendDoc]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[sp_pub_price_SendDoc]
GO
...
/*发送报价单*/
CREATE PROCEDURE [dbo].[sp_pub_price_SendDoc]
(
	@Operator BIGINT,
	@DocID BIGINT
)
...
BEGIN
	...
	-- 合约报价时某项价格为0时不允许发送
	IF @Result = 0 AND (@DocType = 2) AND EXISTS(SELECT [Detail_ID] FROM Price_DocDetails WHERE Detail_DocID = @DocID AND 
