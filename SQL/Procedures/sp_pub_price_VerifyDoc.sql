...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_pub_price_VerifyDoc]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[sp_pub_price_VerifyDoc]
GO
...
/*审核报价单*/
CREATE PROCEDURE [dbo].[sp_pub_price_VerifyDoc]
(
	@Operator BIGINT,
	@DocID BIGINT,
	@Op BIGINT,					-- 0 同意 1 拒绝
...
Flow_InsertTime, Flow_Invalid) VALUES(@DocID, @SrcStatus, @DstStatus, @Op, @Description, @Operator, @CompanyID, GETDATE(), 0);
	...
	SELECT @Result AS Verify_Result;
