...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_pub_price_InsurancePrice]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[sp_pub_price_InsurancePrice]
GO
...
保费
*/
CREATE PROCEDURE [dbo].[sp_pub_price_InsurancePrice]
(
	@Operator BIGINT,
	@DocID BIGINT,				-- 报价单号
	@DetailID BIGINT = 0,       -- 价格编号
	@DocType BIGINT,			-- 报价类型，固定为8
...
BEGIN
	...
	IF @CompanyID <= 0
	BEGIN
		SET @Result = -5200071001;
	END
	
	-- 当前用户没有添加客户的权限
	IF @Result = 0 AND NOT EXISTS(SELECT [User_ID] FROM TMS_User WHERE [User_ID] = @Operator AND User_CompanyID = @CompanyID AND User_Invalid = 0)
	BEGIN
		SET @Result = -520007002;
	END
	
	-- 价格类型是否匹配(保费)
	IF @Result = 0 AND @DocType <> 8
	BEGIN
		SET @Result = -520007003;
	END
	IF @Result = 0 AND @Price <= 0
	BEGIN
		SET @Result = -520007007;
	END
