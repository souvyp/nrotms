			...
			{{each data}}
				<tr>
					...
					<td class="title" title="{{$value.code}}" fld="code">
                        {{if $value.type == 1}}
                        	{{if $value.PastDate == "1" }}
                        		<a target="_blank" class="verified" href="SelfSent_edit.aspx?id={{$value.id}}&code=SelfSearch">{{$value.code}}
                        	{{else}}
                        		<a target="_blank" href="SelfSent_edit.aspx?id={{$value.id}}&code=SelfSearch">{{$value.code}}
                        	{{/if}}
                         {{else if $value.type == 3}}
                        	{{if $value.PastDate == "1" }}
                        		<a target="_blank" class="verified" href="SelfSentAddition_edit.aspx?id={{$value.id}}&code=SelfSearch&OrderID={{$value.OrderID}}">{{$value.code}}
                        	{{else}}
                        		<a target="_blank" href="SelfSentAddition_edit.aspx?id={{$value.id}}&code=SelfSearch&OrderID={{$value.OrderID}}">{{$value.code}}
                        	{{/if}}
                        {{else if $value.type == 4}}
                        	{{if $value.PastDate == "1" }}
                        		<a target="_blank" class="verified" href="SelfSentCombined_edit.aspx?id={{$value.id}}&code=SelfSearch">{{$value.code}}
                        	{{else}}
                        		<a target="_blank" href="SelfSentCombined_edit.aspx?id={{$value.id}}&code=SelfSearch">{{$value.code}}
                        	{{/if}}
	                    {{else if $value.type == 5}}
                        	{{if $value.PastDate == "1" }}
                        		<a target="_blank" class="verified" href="SelfSentAddCombined_edit.aspx?id={{$value.id}}&code=SelfSearch">{{$value.code}}
                        	{{else}}
                        		<a target="_blank" href="SelfSentAddCombined_edit.aspx?id={{$value.id}}&code=SelfSearch">{{$value.code}}
                        	{{/if}}    
                        {{/if}}
                    </td>
...
<script type="text/javascript">
    ...
    var reqeustDone = function () {		/*所有JS加载完成以后执行*/
        ...
        InitList($("#demo"), $("#demo .list"), $("#jplist-template").html(), "SelfSearch", "/Widget.aspx?param=jplist&vid=grid_jplist_0103");
