		...
		<table class="jptable table" id="demoList">
			...
			{{each data}}
				<tr>
					...
					<td class="title" fld="Code" title="{{$value.Code}}"><a href="SelfByOrder_edit.aspx?id={{$value.id}}" >{{$value.Code}}</a></td>
					...
					<td class="title" title="{{$value.To}}" fld="To">{{$value.To}}</td>
		          	...
		          	<td class="title" title="{{$value.SupplierSymbolName}}" fld="SupplierSymbolName">
		          		{{if $value.SrcClass == "2"}}
		          			{{$value.SupplierSymbolName}}
		          		{{/if}}
          			</td>
				</tr>
			{{/each}}
...
<script type="text/javascript">
    ...
    var reqeustDone = function () {		/*所有JS加载完成以后执行*/
        ...
        InitList($("#demo"), $("#demo .list"), $("#jplist-template").html(), "SelfByOrder", "/Widget.aspx?param=jplist&vid=grid_jplist_0098");
