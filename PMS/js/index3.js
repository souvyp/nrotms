...
function CompactSave(that, code ){ 
	...
	if(pricQj){
        ...
        var flag = 0;
        var isReady = tdVal( '#KG tr:not(.hide)', [], 1, 1, 0 );
        if( !isReady ){
            flag = 1;
        }
        isReady = tdVal( '#Ton tr:not(.hide)', [], 1, 2, 0 );
        if( !isReady ){
            flag = 2;
        }
        isReady = tdVal( '#CMetre tr:not(.hide)', [], 2, 3, 0 );
        if( !isReady ){
            flag = 3;
        }
        isReady = tdVal( '#Square tr:not(.hide)', [], 2, 4, 0 );
        if( !isReady ){
            flag = 4;
        }
        isReady = tdVal( '#num tr:not(.hide)', [], 3, $( '.ld' ).find( 'select[name="Unit_id"]' ).val(), 0 );
        if( !isReady ){
            flag = 5;
        }
...
function CompactVal( id, code ){
    ...
    var isReady = tdVal( '#KG tr:not(.hide)', Total, 1, 1, id );
    if( !isReady ){
        flag = 1;
    }
    isReady = tdVal( '#Ton tr:not(.hide)', Total, 1, 2, id );
    if( !isReady ){
        flag = 2;
    }
    isReady = tdVal( '#CMetre tr:not(.hide)', Total, 2, 3, id );
    if( !isReady ){
        flag = 3;
    }
    isReady = tdVal( '#Square tr:not(.hide)', Total, 2, 4, id );
    if( !isReady ){
        flag = 4;
    }
    isReady = tdVal( '#num tr:not(.hide)', Total, 3, $( '.ld' ).find( 'select[name="Unit_id"]' ).val(), id );
    if( !isReady ){
        flag = 5;
    }
    ...
    if( flag == 0 ){   //判断数据是否填满
        for ( var i = 0; i < Total.length; i++ ){
            ...
            {
                json += '{"name":"' + Total[i][j].name + '","value":"' + Total[i][j].value + '"}';
                json += ',';
            }
            json = json.substring( 0, json.length - 1 );
            par += '{"namespace":"protocol","cmd":"data","version":1,"id":"pml_0010","paras":[' + json + ']}';
            par += ',';
            json = '';
        }
        ...
        $.ajax({
            ...
            success: function ( data )
            {
                ...
                if( code == "DocByCompact"){
                    location.href =  'DocByCompact.aspx';
                }else if( code == "MySending"){
                //    location.href = 'MySending.aspx';
...
function tdVal( mark, Total, type, unit, id ){
    //先获取零担重量/公斤内容
    ...
    {	
        ...
        if(  _priceTr.find('input[name="ToName"]').length > 0 ){
            ...
            /*for ( var i = 0; i < title_tdL - 2; i++ )
            {
                if ( $( mark ).eq( index ).find( 'td' ).eq( i + 1 ).find( 'input' ).val() == '' )
                { 
					
				}
                else
                {
                    Prices[i] = $( mark ).eq( index ).find( 'td' ).eq( i + 1 ).find( 'input' ).val();
                }
            }*/

            /*if( Max.length + 1 != Prices.length ){
                alert("缺失数据，请将数据填写完整！");
                isReady = false;
                return;
            }*/
                ...
                if ( Prices[j+1] != '' && Max[j] != "" ){
                    if ( j == 0 ){
                        ...
                        [
                            ...
                            { "name": "ToName", "value": "" + $( mark ).eq( index ).find( 'td' ).eq( 0 ).find( 'input[name="ToName"]' ).val() + "" },
                            ...
                            { "name": "FromName", "value": "" + $( '.ld .province' ).find( 'input[name="From"]' ).val() + "" },
                            { "name": "Max", "value": "" + Max[j] + "" },
                            { "name": "Min", "value": "0" }
                        ];
                    }
                    else{
...
function NoZero( a ) {
    var flag = false;
    $('tr[nrowid="CombineOrderPrice"]').find("input.price").each(function(){
    	var num = $(this).val();
    ...
    if( flag ){
        ...
        if (table.length != 0) {
            SaveFormDataNDT( table, $(this), __saveNdtCfg );
        } else {
            table = $( 'table' );
            $('#IsSend').val(1);
            SaveFormDataNDT( table, $(this), __saveNdtCfg );
...
function sumTotalPrice( input ) {
      ...
      var inputs = $('tr[nrowid="CombineOrderPrice"]').find("input.price");
      var value = 0;
      var totalPrice = 0;
