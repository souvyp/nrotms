			...
			{{each data}}
				<tr>
					...
					<td class="title" title="{{$value.code}}" fld="code">
                        {{if $value.type == 1}}
                        	{{if $value.PastDate == "1" }}
                        		<a target="_blank" class="verified" href="Index_edit.aspx?id={{$value.id}}&code=MySearch">{{$value.code}}
                        	{{else}}
                        		<a target="_blank" href="Index_edit.aspx?id={{$value.id}}&code=MySearch">{{$value.code}}
                        	{{/if}}
                            
                        {{else if $value.type == 2}}
                        	{{if $value.PastDate == "1" }}
                        		<a target="_blank" class="verified" href="DocByCompact_edit.aspx?id={{$value.id}}&code=MySearch">{{$value.code}}
                        	{{else}}
                        		<a target="_blank" href="DocByCompact_edit.aspx?id={{$value.id}}&code=MySearch">{{$value.code}}
                        	{{/if}}
                            
                        {{else if $value.type == 3}}
                        	{{if $value.PastDate == "1" }}
                        		<a target="_blank" class="verified" href="IndexAddition_edit.aspx?id={{$value.id}}&code=MySearch&OrderID={{$value.OrderID}}">{{$value.code}}
                        	{{else}}
                        		<a target="_blank" href="IndexAddition_edit.aspx?id={{$value.id}}&code=MySearch&OrderID={{$value.OrderID}}">{{$value.code}}
                        	{{/if}}
                            
                        {{else if $value.type == 4}}
                        	{{if $value.PastDate == "1" }}
                        		<a target="_blank" class="verified" href="IndexCombined_edit.aspx?id={{$value.id}}&code=MySearch">{{$value.code}}
                        	{{else}}
                        		<a target="_blank" href="IndexCombined_edit.aspx?id={{$value.id}}&code=MySearch">{{$value.code}}
                        	{{/if}}
                        {{else if $value.type == 5}}
                        	{{if $value.PastDate == "1" }}
                        		<a target="_blank" class="verified" href="IndexAddCombined_edit.aspx?id={{$value.id}}&code=MySearch&OrderID={{$value.OrderID}}">{{$value.code}}
                        	{{else}}
                        		<a target="_blank" href="IndexAddCombined_edit.aspx?id={{$value.id}}&code=MySearch&OrderID={{$value.OrderID}}">{{$value.code}}
                        	{{/if}}
                    ...
                    <td class="title"  >
                    	{{if $value.type == "2" }}
                    		<button class="btn btn-link" title="复制" onclick="location.href='DocByCompact.aspx?id={{$value.id}}&copy=1'">复制</button>
                    	{{/if}}
                    </td> 
    			</tr>
			{{/each}}
...
<script type="text/javascript">
    ...
    var reqeustDone = function () {		/*所有JS加载完成以后执行*/
        ...
        InitList($("#demo"), $("#demo .list"), $("#jplist-template").html(), "company", "/Widget.aspx?param=jplist&vid=grid_jplist_0053");
