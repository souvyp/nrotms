...
<html>
<head>		
<title>待发送报价单-<%=NDS.Kits.Config.Instance.WebsiteParameters("ProductName")%></title>
<meta charset="utf-8" />
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer" style="position:absolute; top:0; width:100%; padding-bottom:0;">
                    <button type="button" class="btn btn-default  okButton footKeepBtn" style="margin-right:8px; box-shadow:none;" aria-label="OK" title="确定" onclick="SendDoc()"><span aria-hidden="true" class="">确定</span><span class="glyphicon glyphicon-ok" style="margin-left: 2px;"></span></button>
            ...
            <div class="modal-content">
                <div class="modal-header" style=" padding-bottom:0;padding-top:20px;">
                    ...
                    <h4 class="modal-title text-left" id="H1">
                        <div style="padding-left: 3px; background-color: #f27302;">
                            <p style="background-color: white; padding-left: 8px; height: 20px;margin-bottom:0; line-height: 20px; margin-bottom: 20px;color:#666; font-size:14px;font-family:'微软雅黑';">关闭报价单</p>
                        </div>
                    </h4>
                </div>
                <div class="modal-body">
                ...
                <div class="modal-footer" style="position:absolute; top:0; width:100%; padding-bottom:0;">
					<button type="button" class="btn btn-default  okButton footKeepBtn" style="margin-right:8px; box-shadow:none;" aria-label="OK" title="确定" onclick="CloseDoc()" ><span aria-hidden="true" class="">确定</span><span class="glyphicon glyphicon-ok" style="margin-left:2px;"></span></button>
					<button type="button" class="btn btn-default " data-dismiss="modal" aria-label="Close" title="关闭" style="box-shadow:none;margin-left:0;"><span aria-hidden="true" class="">关闭</span><span class="glyphicon glyphicon-remove" style="margin-left:2px;color:#888;"></span></button>
                </div>
            </div>
            <!-- /.modal-content -->
		...
		<table class="jptable table" id="demoList">
			...
			{{each data}}
				<tr>
					...
					<td class="title" title="{{$value.code}}" fld="code" title="{{$value.code}}">
                        {{if $value.type == 1}}
                            <a  href="MySending_edit.aspx?id={{$value.id}}" title="{{$value.code}}">{{$value.code}}
                        {{else if $value.type == 2}}
                            <a  href="SendCompact_edit.aspx?id={{$value.id}}" title="{{$value.code}}">{{$value.code}}
                        {{else if $value.type == 3}}
                            <a  href="AdditionSend_edit.aspx?id={{$value.id}}&OrderID={{$value.OrderID}}" title="{{$value.code}}">{{$value.code}}
                        {{else if $value.type == 4}}
                            <a  href="SendingCombined_edit.aspx?id={{$value.id}}" title="{{$value.code}}">{{$value.code}}
                        {{else if $value.type == 5}}
                            <a  href="SendAddCombined_edit.aspx?id={{$value.id}}&OrderID={{$value.OrderID}}" title="{{$value.code}}">{{$value.code}}
                        {{/if}}
                    </td>
                    <td class="title" title="{{$value.OrderCode}}" fld="OrderCode">{{$value.OrderCode}}</td>
                    <td class="title" title="{{$value.PactCode}}" fld="PactCode" title="{{$value.PactCode}}">{{$value.PactCode}}</td>
 					<td class="title" title="{{$value.customercompanyname}}" fld="customercompanyname" title="{{$value.customercompanyname}}">{{$value.customercompanyname}}</td>
                   <td class="title" title="{{$value.starttime}}" fld="starttime" title="{{$value.starttime}}">{{$value.starttime}}</td>
                    <td class="title" title="{{$value.endtime}}" fld="endtime" title="{{$value.endtime}}">{{$value.endtime}}</td>                    
                    <td class="title" title="{{$value.type}}" fld="type" title="{{$value.type}}">
                        {{if $value.type == "1"}}
                            按单报价
                        {{else if $value.type == "2" }}
                            合约
                    ...
                    <td class="title" title="{{$value.StatusName}}" fld="StatusName" title="{{$value.StatusName}}">{{$value.StatusName}}</td> 
                    <td class="title">
                        <button class="btn btn-link" title="发送报价单"  onclick="IsSend({{$value.id}})" style="margin-left:0;">发送</button>
                        <button class="btn btn-link" title="关闭报价单" onclick="Closed({{$value.id}})">关闭</button>
                    </td>
    			</tr>
			{{/each}}
			</tbody>
...
<!--<><><><><><><><><><><><><><><><><><><><><><><><><><> 数据列表开始 <><><><><><><><><><><><><><><><><><><><><><><><><><>-->
		<div id="demo" class="box jplist">
			...
			<div class="jplist-panel box panel-top min_height">	
				...
				<input data-path="PactCode" data-button="#sopt_status-search-button" type="text" value="" placeholder="合同编号" data-control-type="textbox" data-control-name="PactCode" data-control-action="filter">
				</div>
				...
				<div class="text-filter-box"><!--[if lt IE 10]><div class="jplist-label">结束时间:</div><![endif]-->
					<input readonly data-path="comparefilter_le_endtime" onclick="GetDateEvent(this, { format: 'YYYY/MM/DD', min: $('input[data-path=\'comparefilter_ge_starttime\']').val() })" data-button="#search-button" type="text" value="" placeholder="结束时间" data-control-type="textbox" data-control-name="comparefilter_le_endtime" data-control-action="filter"> 
				...
				<div class="collapse clearfix" id="collapseExample" style="margin-left:75px;">
					...
					<div style="" class="jplist-drop-down" data-control-type="filter-drop-down" data-control-name="type" data-control-action="filter">					
						<ul>
							<li><span data-path="default">报价方式</span></li>
							<li><span data-path=1>按单报价</span></li>
							<li><span data-path=2 >合约</span></li>
							<li><span data-path=3 >补充报价</span></li>
							<li><span data-path=4 >合单报价</span></li>							
...
<script type="text/javascript">
    ...
    var reqeustDone = function () {		/*所有JS加载完成以后执行*/
        ...
        InitList($("#demo"), $("#demo .list"), $("#jplist-template").html(), "company", "/Widget.aspx?param=jplist&vid=grid_jplist_0048");
