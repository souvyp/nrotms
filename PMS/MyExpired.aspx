			...
			{{each data}}
				<tr>
					...
					<td class="title" title="{{$value.code}}" fld="code">
                        {{if $value.type == 1}}
                            <a target="_blank" href="Index_edit.aspx?id={{$value.id}}&code=MyExpired">{{$value.code}}
                        {{else if $value.type == 2}}
                            <a target="_blank" href="DocByCompact_edit.aspx?id={{$value.id}}&code=MyExpired">{{$value.code}}
                        {{else if $value.type == 3}}
                            <a target="_blank" href="IndexAddition_edit.aspx?id={{$value.id}}&code=MyExpired">{{$value.code}}
                        {{else if $value.type == 4}}
                            <a target="_blank" href="SendingCombined_edit.aspx?id={{$value.id}}&code=MyExpired">{{$value.code}}
                        {{/if}}                    
                    </td>
...
<script type="text/javascript">
    ...
    var reqeustDone = function () {		/*所有JS加载完成以后执行*/
        ...
        InitList($("#demo"), $("#demo .list"), $("#jplist-template").html(), "company", "/Widget.aspx?param=jplist&vid=grid_jplist_0049");
