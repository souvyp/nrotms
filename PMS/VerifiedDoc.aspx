...
<html>
<head>		
<title>已审核报价单-<%=NDS.Kits.Config.Instance.WebsiteParameters("ProductName")%></title>
<meta charset="utf-8" />
		...
		<table class="jptable table" id="demoList">
			...
			{{each data}}
				<tr>
					...
					<td class="title" fld="code">
                        {{if $value.type == 1}}
                            <a target="_blank" href="Index_edit.aspx?id={{$value.id}}&read=0&code=VerifiedDoc">{{$value.code}}
                        {{else if $value.type == 2}}
                            <a target="_blank" href="DocByCompact_edit.aspx?id={{$value.id}}&read=0&code=VerifiedDoc">{{$value.code}}
                        {{else if $value.type == 3}}
                            <a target="_blank" href="IndexAddition_edit.aspx?id={{$value.id}}&read=0&code=VerifiedDoc&OrderID={{$value.OrderID}}">{{$value.code}}
                        {{else if $value.type == 4}}
	                        <a target="_blank" href="IndexCombined_edit.aspx?id={{$value.id}}&read=0&code=VerifiedDoc">{{$value.code}}
                        {{else if $value.type == 5}}
	                        <a target="_blank" href="IndexAddCombined_edit.aspx?id={{$value.id}}&code=VerifiedDoc&OrderID={{$value.OrderID}}">{{$value.code}}
                        {{/if}}
                    </td>
                    ...
                    <td class="title" fld="supplierCompanyname"  title="{{$value.supplierCompanyname }}">{{$value.supplierCompanyname }}</td>
                    <td class="title" fld="starttime" title="{{$value.starttime}}">{{$value.starttime}}</td>
                    <td class="title" fld="endtime" title="{{$value.endtime}}">{{$value.endtime}}</td>                    
                    ...
                    <td class="title" fld="StatusName" style="color:#30AD5A !important;">{{$value.StatusName}}</td> 
    			</tr>
			{{/each}}
			</tbody>
		</table>
...
<script type="text/javascript">
    ...
    var reqeustDone = function () {		/*所有JS加载完成以后执行*/
        ...
        InitList($("#demo"), $("#demo .list"), $("#jplist-template").html(), "company", "/Widget.aspx?param=jplist&vid=grid_jplist_0054");
        
        setTimeout('autoSearch()', 200);
    ...
    function autoSearch()
    {
        if (getQueryString('supplierCompanyname')) {
            $('input[data-path="supplierCompanyname"]').val(getQueryString('supplierCompanyname'));
            $('#search-button').click();
