...
<html>
<head>		
<title>已发送报价单-<%=NDS.Kits.Config.Instance.WebsiteParameters("ProductName")%></title>
<meta charset="utf-8" />
<meta name="viewport" content="width=device-width, initial-scale=1" />
<!--#include file="/Controls/Meta.aspx"-->
<!--#include file="/Controls/PMS/CSS.aspx"-->
<link href="<%=MinifyUrl("ListCss")%>" rel="stylesheet" type="text/css" />
<link href="city/css/cityLayoutlist.css" type="text/css" rel="stylesheet">
</head>
<body code="Index" role="Index">
<style type="text/css">
			...
			{{each data}}
				<tr>
					...
					<td class="title" title="{{$value.code}}" fld="code">
                        {{if $value.type == 1}}
                            <a target="_blank" href="Index_edit.aspx?id={{$value.id}}&code=Index">{{$value.code}}
                        {{else if $value.type == 2}}
                            <a target="_blank" href="DocByCompact_edit.aspx?id={{$value.id}}&code=Index">{{$value.code}}
                        {{else if $value.type == 3}}
                            <a target="_blank" href="IndexAddition_edit.aspx?id={{$value.id}}&code=Index&OrderID={{$value.OrderID}}">{{$value.code}}
                        {{else if $value.type == 4}}
	                        <a target="_blank" href="IndexCombined_edit.aspx?id={{$value.id}}&code=Index">{{$value.code}}
	                    {{else if $value.type == 5}}
	                        <a target="_blank" href="IndexAddCombined_edit.aspx?id={{$value.id}}&code=Index&OrderID={{$value.OrderID}}">{{$value.code}}
                        {{/if}}
                    </td>
                    ...
                    <td class="title" >
                        {{if $value.StatusName == "已审核" && $value.type == "1" }}
                        <button type="button" class="btn btn-link" onclick="reDocPrice({{$value.id}})" title="重新报价">重新报价</button>
                        {{/if}}
                    </td>
    			</tr>
			{{/each}}
...
<!--<><><><><><><><><><><><><><><><><><><><><><><><><><> 数据列表开始 <><><><><><><><><><><><><><><><><><><><><><><><><><>-->
		<div id="demo" class="box jplist">
			...
			<div class="jplist-panel box panel-top min_height">				
				...
				<div style="" class="jplist-drop-down" data-control-type="filter-drop-down" data-control-name="StatusName" data-control-action="filter">					
					<ul>
						<li><span data-path="default">报价状态</span></li>
						<li><span data-path="已审核">已审核</span></li>
						<li><span data-path="待审核" >待审核</span></li>
					</ul>	
				</div>						
				...
				<div class="collapse clearfix" id="collapseExample" style="margin-left:75px;">
					...
					<div class=""  style="height:30px;display:none;float:left;padding:10px 10px 0px 0px">
                        <input type="text" readonly placeholder="起点城市(省/市/区)" class="city_input inputFocus proCitySelAll"/>
                        <input name="FromProvince" type="hidden" class="edit form-control transparent"  data-path="FromProvince" data-button="#search-button" data-control-type="textbox" data-control-name="FromProvince" data-control-action="filter">
                        <input name="FromCity" type="hidden" class="edit form-control transparent"  data-path="FromCity" data-button="#search-button" data-control-type="textbox" data-control-name="FromCity" data-control-action="filter">
                        <input name="FromDistrict" type="hidden" class="edit form-control transparent"  data-path="FromDistrict" data-button="#search-button" data-control-type="textbox" data-control-name="FromDistrict" data-control-action="filter">                
...
<script type="text/javascript">
    ...
    var reqeustDone = function () {		/*所有JS加载完成以后执行*/
        ...
        InitList($("#demo"), $("#demo .list"), $("#jplist-template").html(), "company", "/Widget.aspx?param=jplist&vid=grid_jplist_0047");
...
<script src="city/js/publiclist.js"></script>
