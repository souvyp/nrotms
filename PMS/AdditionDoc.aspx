...
<div class="modal fade" id="rangeModal" tabindex="-1" role="dialog" 
 ...
 <!--查看费用-->
    <div class="modal" id="searchCost" tabindex="-1" role="dialog"
        aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog" style="width:80%">
            <div class="modal-content">
                <div class="modal-header" style=" padding-bottom:0;padding-top:20px;">                    
                    <h4 class="modal-title text-left" id="myModalLabel">
                        <div style="padding-left: 3px; background-color: #f27302;">
                            <p style="background-color: white; padding-left: 8px; height: 20px;margin-bottom:0; line-height: 20px; margin-bottom: 20px;color:#666; font-size:14px;font-family:'微软雅黑';">查看费用</p>
                        </div>
                    </h4>
                </div>
                <div class="modal-body">
                	<table>
                		<thead>
                			<tr style="height: 26px;background-color:#FAFBFD;">
						      <td class="title text-left" style="padding-left:6px"  colspan="">零担</td>
						      <td class="title text-left" style="padding-left:6px" colspan="">整车</td>
						      <td class="title text-left" style="padding-left:6px" colspan="">提货费</td>
						      <td class="title text-left" style="padding-left:6px" colspan="">送货费</td>
						      <td class="title text-left" style="padding-left:6px" colspan="">装货费</td>
						      <td class="title text-left" style="padding-left:6px" colspan="">卸货费</td>
						      <td class="title text-left" style="padding-left:6px" colspan="">保险费</td>
						      <td class="title text-left" style="padding-left:6px" colspan="">税费</td>
						      <td class="title text-left" style="padding-left:6px" colspan="">附加费</td>
						      <td class="title text-left" style="padding-left:6px" colspan="">合计</td>
						    </tr>
                		</thead>
                ...
                <div class="modal-footer" style="position:absolute; top:0; width:100%; padding-bottom:0;"><!--ScheduleOrder(0)-->
                    <button type="button" class="btn btn-default cancer" data-dismiss="modal" aria-label="Close" title="关闭" style="box-shadow:none;margin-left:0;"><span aria-hidden="true" class="">关闭</span><span class="glyphicon glyphicon-remove" style="margin-left: 2px; color: #888;"></span></button>
                </div>
            </div>
            <!-- /.modal-content -->
        </div>
		...
		<table class="jptable table" id="demoList">
			...
			{{each data}}
				<tr>
					...
					<td class="title" fld="Code" title="{{$value.Code}}"><a  href="AdditionDoc_edit.aspx?id={{$value.id}}" >{{$value.Code}}</a></td>
					...
					<td class="title" title="{{$value.IFrom}}" fld="IFrom">{{$value.IFrom}}</td>
					<td class="title" title="{{$value.ITo}}" fld="ITo">{{$value.ITo}}</td>
		          <td class="title" title="{{$value.TotalAmount}}" fld="TotalAmount">{{$value.TotalAmount}}</td>
		          <td class="title" title="{{$value.TotalWeight}}" fld="TotalWeight">{{$value.TotalWeight}}</td>
		          <td class="title" title="{{$value.TotalVolume}}" fld="TotalVolume">{{$value.TotalVolume}}</td>
		          <td class="title"  fld="TotalCost"><span>{{$value.TotalCost}}</span></td>
				</tr>
			{{/each}}
			</tbody>
		</table>
...
<!--<><><><><><><><><><><><><><><><><><><><><><><><><><> 数据列表开始 <><><><><><><><><><><><><><><><><><><><><><><><><><>-->
		<div id="demo" class="box jplist">
            ...
            <div class="maintitle_container">
               <p class="mainHtml_tit">补充报价</p>
            </div>
			<!-- 操作面板 -->
			<div class="jplist-panel box panel-top min_height">
                <button class="reset" type="button" class="jplist-reset-btn" data-control-type="reset" data-control-name="reset" data-control-action="reset">重置 &nbsp;<i class="glyphicon glyphicon-refresh"></i></button>
...
<script type="text/javascript">
    ...
    var reqeustDone = function () {		/*所有JS加载完成以后执行*/
        ...
        InitList($("#demo"), $("#demo .list"), $("#jplist-template").html(), "OrderAccept", "/Widget.aspx?param=jplist&vid=grid_jplist_0051");

        resizeCols = setInterval('resizeTable()', 200);

       $(document).on('mouseover mouseout', 'td>span', function(e) {
       		if (e.type == 'mouseover' ) {
       			$('div#searchCost').modal('show');
       			OrderPriceShow($(this).closest('tr').find('input[name="id"]').val());
       		} 
        });
    };
