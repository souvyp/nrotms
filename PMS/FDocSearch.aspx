...
<html>
<head>		
<title>订单列表-<%=NDS.Kits.Config.Instance.WebsiteParameters("ProductName")%></title>
<meta charset="utf-8" />
		...
		<table class="jptable table" id="demoList">
			...
			{{each data}}
				<tr>
					...
					<td class="title" fld="Code" title="{{$value.Code}}"><a target="_blank" href="FDocSearch_edit.aspx?id={{$value.id}}" >{{$value.Code}}</a></td>
					...
					<td class="title" title="{{$value.ITo}}" fld="ITo">{{$value.ITo}}</td>
          			...
          			<td class="title" title="{{$value.DocCode}}" fld="DocCode">{{$value.DocCode}}</td>
				</tr>
			{{/each}}
			</tbody>
		</table>
...
<!--<><><><><><><><><><><><><><><><><><><><><><><><><><> 数据列表开始 <><><><><><><><><><><><><><><><><><><><><><><><><><>-->
		<div id="demo" class="box jplist">
			...
			<div class="jplist-panel box panel-top min_height">
				...
				<div class="text-filter-box"><!--[if lt IE 10]><div class="jplist-label">编码:</div><![endif]--><input data-path="DocCode" data-button="#opt_status-search-button" type="text" value="" placeholder="报价单编码" data-control-type="textbox" data-control-name="DocCode" data-control-action="filter"> </div>
				...
				<div class="text-filter-box">
                    <input data-path="rangefilter_createtime" class="rangefilter" data-button="#search-button" type="text" value="" placeholder="制单时段" data-control-type="textbox" data-control-name="rangefilter_createtime" data-control-action="filter">
...
<script type="text/javascript">
    ...
    var reqeustDone = function () {		/*所有JS加载完成以后执行*/
        ...
        InitList($("#demo"), $("#demo .list"), $("#jplist-template").html(), "OrderAccept", "/Widget.aspx?param=jplist&vid=grid_jplist_0115");
