		...
		<table class="jptable table">
			...
			{{each data}}
				<tr>
					...
					<td class="title" fld="code"><a target="_blank" href="MySent_edit.aspx?id={{$value.id}}">{{$value.code}}</td>
					<td class="title" fld="name">{{$value.name}}</td>                    
                    <td class="title" fld="starttime">{{$value.starttime}}</td>
                    <td class="title" fld="endtime">{{$value.endtime}}</td>                    
...
<!--<><><><><><><><><><><><><><><><><><><><><><><><><><> 数据列表开始 <><><><><><><><><><><><><><><><><><><><><><><><><><>-->
		<div id="demo" class="box jplist">
            ...
            <div class="maintitle_container">
               <p class="mainHtml_tit">已发送报价单列表</p>
            </div>
			<!-- 操作面板 -->
			<div class="jplist-panel box panel-top min_height">				
				...
				<div class="jplist-drop-down" data-control-type="sort-drop-down" data-control-name="sort" data-control-action="sort">
					<ul>
						...
						<li><span data-path="CompanyName" data-order="asc" data-type="text">报价单名称</span></li>
                    </ul>
				</div>
