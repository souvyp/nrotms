		...
		<table class="jptable table">
			...
			{{each data}}
				<tr>
					...
					<td class="title" fld="Code" title="{{$value.Code}}"><a  href="DocCombined_edit.aspx?id={{$value.id}}" >{{$value.Code}}</a></td>
			        ...
			        <td class="title" title="{{$value.SupplierName}}" fld="SupplierName">
			          	{{if $value.SupplierName !="自营" && $value.SupplierName !="" }}
			          		{{$value.SupplierName}}
			          	{{/if}}
			        </td>
				</tr>
...
<script type="text/javascript">
    var reqeustDone = function () {		/*所有JS加载完成以后执行*/
        ...
        InitList($("#demo"), $("#demo .list"), $("#jplist-template").html(), "OrderAccept", "/Widget.aspx?param=jplist&vid=grid_jplist_0085");
    };
