...
<html>
<head>		
<title>待审核报价单-<%=NDS.Kits.Config.Instance.WebsiteParameters("ProductName")%></title>
<meta charset="utf-8" />
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer" style="position:absolute; top:0; width:100%; padding-bottom:0;">
				<button type="button" class="btn btn-default  okButton footKeepBtn" style="margin-right:8px; box-shadow:none;" aria-label="OK" title="确定" onclick="VerifyDoc()" ><span aria-hidden="true" class="">确定</span><span class="glyphicon glyphicon-ok" style="margin-left:2px;"></span></button>
			...
			{{each data}}
				<tr>
					...
					<td class="title" fld="code">
                        {{if $value.type == 1}}
                            <a href="Index_edit.aspx?id={{$value.id}}&status=1&code=VerifingDoc">{{$value.code}}
                        {{else if $value.type == 2}}
                            <a  href="DocByCompact_edit.aspx?id={{$value.id}}&status=1&code=VerifingDoc">{{$value.code}}
                        {{else if $value.type == 3}}
                            <a  href="IndexAddition_edit.aspx?id={{$value.id}}&status=1&code=VerifingDoc&OrderID={{$value.OrderID}}">{{$value.code}}
                        {{else if $value.type == 4}}
	                        <a  href="SendingCombined_edit.aspx?id={{$value.id}}&status=1&code=VerifingDoc">{{$value.code}}
                        {{else if $value.type == 5}}
	                        <a  href="IndexAddCombined_edit.aspx?id={{$value.id}}&status=1&code=VerifingDoc&OrderID={{$value.OrderID}}">{{$value.code}}
                        {{/if}}
                    </td>
                    ...
                    <td class="title" fld="starttime" title="{{$value.endtime}}">{{$value.starttime}}</td>
                    ...
                    <td class="title" fld="StatusName" title="{{$value.StatusName}}">{{$value.StatusName}}</td> 
                    <td class="title">
                        <button class="btn btn-link mainColor" title="同意" onclick="VerifyDocOk({{$value.id}})" style="margin-left:0;">同意</button>
                        <button class="btn btn-link" title="拒绝" onclick="VerifyDocNo({{$value.id}})">拒绝</button>
                    </td>
    			</tr>
			{{/each}}
			</tbody>
...
<script type="text/javascript">
    ...
    var reqeustDone = function () {		/*所有JS加载完成以后执行*/
        ...
        InitList($("#demo"), $("#demo .list"), $("#jplist-template").html(), "company", "/Widget.aspx?param=jplist&vid=grid_jplist_0055");
