...
<html>
<head>		
<title>报价单检索-<%=NDS.Kits.Config.Instance.WebsiteParameters("ProductName")%></title>
<meta charset="utf-8" />
			...
			{{each data}}
				<tr>
					...
					<td class="title" title="{{$value.code}}" fld="code">
                        {{if $value.type == 1}}
                        	{{if $value.PastDate == "1" }}
                        		<a target="_blank" class="verified" href="Index_edit.aspx?id={{$value.id}}&code=VerifySearch">{{$value.code}}</a>
                        	{{else}}
                        		<a target="_blank"  href="Index_edit.aspx?id={{$value.id}}&code=VerifySearch">{{$value.code}}</a>
                        	{{/if}}
                        {{else if $value.type == 2}}
                        	{{if $value.PastDate == "1" }}
                        		<a target="_blank" class="verified" href="DocByCompact_edit.aspx?id={{$value.id}}&code=VerifySearch">{{$value.code}}</a>
                        	{{else}}
                        		<a target="_blank" href="DocByCompact_edit.aspx?id={{$value.id}}&code=VerifySearch">{{$value.code}}</a>
                        	{{/if}}
                        
                        {{else if $value.type == 3}}
                        	{{if $value.PastDate == "1" }}
                        		<a target="_blank" class="verified" href="IndexAddition_edit.aspx?id={{$value.id}}&code=VerifySearch&OrderID={{$value.OrderID}}">{{$value.code}}</a>
                        	{{else}}
                        		<a target="_blank" href="IndexAddition_edit.aspx?id={{$value.id}}&code=VerifySearch&OrderID={{$value.OrderID}}">{{$value.code}}</a>
                        	{{/if}}
                        
                        {{else if $value.type == 4}}
                        	{{if $value.PastDate == "1" }}
                        		<a target="_blank" class="verified" href="IndexCombined_edit.aspx?id={{$value.id}}&code=VerifySearch">{{$value.code}}</a>
                        	{{else}}
                        		<a target="_blank" href="IndexCombined_edit.aspx?id={{$value.id}}&code=VerifySearch">{{$value.code}}</a>
                        	{{/if}}
                        {{else if $value.type == 5}}
                        	{{if $value.PastDate == "1" }}
                        		<a target="_blank" class="verified" href="IndexAddCombined_edit.aspx?id={{$value.id}}&code=VerifySearch&OrderID={{$value.OrderID}}">{{$value.code}}</a>
                        	{{else}}
                        		<a target="_blank" href="IndexAddCombined_edit.aspx?id={{$value.id}}&code=VerifySearch&OrderID={{$value.OrderID}}">{{$value.code}}</a>
                        	{{/if}}
                        {{/if}}
                    </td>
...
<!--<><><><><><><><><><><><><><><><><><><><><><><><><><> 数据列表开始 <><><><><><><><><><><><><><><><><><><><><><><><><><>-->
		<div id="demo" class="box jplist">
			...
			<div class="jplist-panel box panel-top min_height">	
				...
				<div class="text-filter-box"><!--[if lt IE 10]><div class="jplist-label">姓名:</div><![endif]--><input data-path="name" data-button="#opt_status-search-button" type="text" value="" placeholder="报价单名称" data-control-type="textbox" data-control-name="name" data-control-action="filter"></div>
...
<script type="text/javascript">
    ...
    var reqeustDone = function () {		/*所有JS加载完成以后执行*/
        ...
        InitList($("#demo"), $("#demo .list"), $("#jplist-template").html(), "company", "/Widget.aspx?param=jplist&vid=grid_jplist_0057");
