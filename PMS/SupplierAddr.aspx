...
<html>
<head>		
<title>承运方常用地址-<%=NDS.Kits.Config.Instance.WebsiteParameters("ProductName")%></title>
<meta charset="utf-8" />
...
<!-- 通用对话框开始-->
<div class="modal fade text-center" id="win-Common-Dialog" tabindex="-1" 承运="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		...
		<table class="jptable table">
			...
			{{each data}}
				<tr>
					...
					<td class="title" fld="CustomerName"><a target="_blank" href="SupplierAddr_edit.aspx?id={{$value.id}}">{{$value.CustomerName}}</a></td>
...
<!--<><><><><><><><><><><><><><><><><><><><><><><><><><> 数据列表开始 <><><><><><><><><><><><><><><><><><><><><><><><><><>-->
		<div id="demo" class="box jplist">
            ...
            <div class="maintitle_container">
               <p class="mainHtml_tit">承运方常用地址</p>
            </div>
			<!-- 操作面板 -->
			<div class="jplist-panel box panel-top min_height">
				<button type="button" class="form_sub" onclick="javascript:location.href='SupplierAddr_edit.aspx'">新增&nbsp;<i class="glyphicon glyphicon-plus-sign"></i></button>
				...
				<div class="jplist-drop-down" data-control-type="sort-drop-down" data-control-name="sort" data-control-action="sort">
					<ul>
						...
						<li><span data-path="CustomerName" data-order="asc" data-type="text">承运方名称</span></li>
					</ul>
				</div>
			<div class="text-filter-box" style="display:none;"><input data-path="opt_status" type="text" value="0" data-button="#opt_status-search-button" data-control-type="textbox" data-control-name="opt_status" data-control-action="filter"/><button type="button" id="opt_status-search-button"></button></div>
				<div class="text-filter-box"><input data-path="dbo.fn_pub_user_AddrCompany2Name(3, Addr_CustomerID)" data-button="#name-search-button" type="text" value="" placeholder="承运方名称" data-control-type="textbox" data-control-name="dbo.fn_pub_user_AddrCompany2Name(3, Addr_CustomerID)" data-control-action="filter"> <button type="button" id="name-search-button"><i class="fa glyphicon glyphicon-search"></i></button></div>
				<!-- 分页结果 -->
...
<script type="text/javascript">
    var reqeustDone = function () {		/*所有JS加载完成以后执行*/
        ...
        InitList($("#demo"), $("#demo .list"), $("#jplist-template").html(), "rjwqhugn", "/Widget.aspx?param=jplist&vid=grid_jplist_0022");
    };
