...
<body code="SelfSent" role="SelfSent">
<style type="text/css">
		...
		<table class="jptable table" id="demoList">
			...
			<thead>
				<tr class="trtitle">
					...
					<td style="width:155px;" class="title">报价单编码</td>
					<td class="title">单据编号</td>
					<td style="width:155px;" class="title">合同编号</td>
			...
			{{each data}}
				<tr>
					...
					<td class="title" title="{{$value.code}}" fld="code">
                        {{if $value.type == 1}}
                            <a target="_blank" href="SelfSent_edit.aspx?id={{$value.id}}&code=SelfSent">{{$value.code}}
                        {{else if $value.type == 3}}
	                        <a target="_blank" href="SelfSentAddition_edit.aspx?id={{$value.id}}&code=SelfSent&OrderID={{$value.OrderID}}">{{$value.code}}    
                        {{else if $value.type == 4}}
	                        <a target="_blank" href="SelfSentCombined_edit.aspx?id={{$value.id}}&code=SelfSent">{{$value.code}}
	                    {{else if $value.type == 5}}
	                        <a target="_blank" href="SelfSentAddCombined_edit.aspx?id={{$value.id}}&code=SelfSent&OrderID={{$value.OrderID}}">{{$value.code}}
                        {{/if}}
                    </td>
...
<script type="text/javascript">
    ...
    var reqeustDone = function () {		/*所有JS加载完成以后执行*/
        ...
        InitList($("#demo"), $("#demo .list"), $("#jplist-template").html(), "SelfSent", "/Widget.aspx?param=jplist&vid=grid_jplist_0100");
