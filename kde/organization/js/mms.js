...
MM.Item = function() {
	...
	this._dom.photo.classList.add("photo");
	this._dom.drops.classList.add("drop-item");
	...
	if(DropItem)
		DropItem(this);
//	this._context=document.querySelector("#context");
}
...
MM.Item.prototype.toJSON = function() {
	...
	if (this._photo) { data.photo = this._photo; }
	if (this._drops) { data.drops = this._drops; }
	...
	if (this._others){data.others=this._others;}
	if (this._data) {data.data=this._data;}
...
MM.Item.prototype.fromJSON = function(data) {
	...
	if (data.photo) { this._photo = data.photo; }
	if (data.drops) { this._drops = data.drops; }
	...
	if (data.others) { this._others = data.others; }
...
MM.Item.prototype.mergeWith = function(data) {
	...
	if (this._photo != data.photo) { 
		this._photo = data.photo;
		dirty = 1;
	}

	if (this._drops != data.drops) { 
...
//	this._context.innerHTML=this.getText();
	return this;
}
...
MM.Item.prototype.setPhoto = function(photo) {
	this._photo = photo;
	return this.update();
}

MM.Item.prototype.getPhoto = function() {
	return this._photo;
}

MM.Item.prototype.setDrops = function(drops) {
	this._drops = drops;
	return this.update();
}

MM.Item.prototype.getDrops = function() {
	return this._drops;
}
...
MM.Item.prototype._updatePhoto = function() {
	this._dom.photo.className = "photo";
	var photo = this._photo;
	if(photo){
		this._dom.photo.style.display = "";
		this._dom.photo.src=photo;
	}
	else{
		this._dom.photo.style.display = "none";
		this._dom.photo.src='';
	}
}

MM.Item.prototype._updateDrops = function() {
	var drops = this._drops;
	if(drops){
		this._dom.drops.style.display = "";
		this._dom.drops.innerHTML='<span class="glyphicon glyphicon-grain"></span>'+drops;
	}
	else{
		this._dom.drops.style.display = "none";
		this._dom.drops.innerHTML='';
	}
}
...
MM.Keyboard.handleEvent = function(e) {
	...
	while (node && node != document) {
		if (node.classList.contains("ui") || node.classList.contains("modal")) { return; }
		node = node.parentNode;
	}
...
MM.Action.SetPhoto = function(item, photo) {
	this._item = item;
	this._photo = photo;
	this._oldPhoto = item.getPhoto();
}
MM.Action.SetPhoto.prototype = Object.create(MM.Action.prototype);
MM.Action.SetPhoto.prototype.perform = function() {
	this._item.setPhoto(this._photo);
}
MM.Action.SetPhoto.prototype.undo = function() {
	this._item.setPhoto(this._oldPhoto);
}
...
MM.Menu = {
	...
	open: function(x, y) {
		this._dom.node.style.display = "block";
...
MM.Command.InsertSibling = Object.create(MM.Command, {
	label: {value: "Insert Sibling Node"},
...
MM.Command.New.execute = function() {
	if (!confirm("确实扔掉你的当前的导图并开始一个新的导图吗?")) { return; }
	var map = new MM.Map();
	MM.App.setMap(map);
...
MM.Command.Resource = Object.create(MM.Command, {
	label: {value: "Show/hide resource"},
	keys: {value: [{charCode: "?".charCodeAt(0)}]}
});
MM.Command.Resource.execute = function() {
	MM.App.resource.toggle();
}
...
MM.Command.Value.execute = function() {
	...
	var newValue = prompt("Set Node Value", oldValue?oldValue:'');
...
MM.Layout._alignItem = function(item, side) {
	...
	switch (side) {
		case "left":
			...
			dom.content.appendChild(dom.photo);
			dom.content.insertBefore(dom.drops, dom.content.firstChild);
		break;
		case "right":
			dom.content.insertBefore(dom.value, dom.content.firstChild);
			dom.content.insertBefore(dom.status, dom.content.firstChild);
			dom.content.insertBefore(dom.photo, dom.content.firstChild);
			dom.content.appendChild(dom.drops);
...
MM.Format.JSON = Object.create(MM.Format, {
	...
	extension: {value: "mmp"},
	mime: {value: "application/vnd.mmp+json"}
});
...
MM.Format.Plaintext = Object.create(MM.Format, {
	...
	extension: {value: "txt"},
	mime: {value: "application/vnd.mmp+txt"}
});
...
MM.UI.prototype.handleEvent = function(e) {
	switch (e.type) {
		case "click":
			...
			if ( nodeName != "select" && nodeName != "input" && nodeName != "textarea") { MM.Clipboard.focus(); } /* focus the clipboard (2c) */
...
MM.UI.Photo = function() {
	this._photo = document.querySelector("#photo");
	this._photo.addEventListener("click", this);
}

MM.UI.Photo.prototype.update = function() {	
	this._photo.value = MM.App.current.getPhoto() || "";
}

MM.UI.Photo.prototype.handleEvent = function(e) {
	var action = new MM.Action.SetPhoto(MM.App.current, this._photo.value || null);
	MM.App.action(action);
}
...
MM.UI.Resource = function() {
	this._node = document.querySelector("#resource");
}

MM.UI.Resource.prototype.toggle = function() {
	this._node.classList.toggle("visible");
}
...
MM.UI.IO.prototype.show = function(mode) {
	...
	if(mode=="save")
		this._heading.innerHTML = '保存';
	else
		this._heading.innerHTML = '打开';
...
MM.UI.IO.prototype._setCurrentBackend = function(backend) {
	...
	try {
		//this._updateURL(); /* fails when on file:/// not update by mcfee 15/3/23 */
	} catch (e) {}
}
...
MM.UI.Backend._error = function(e) {
	...
	tip.innerHTML="IO error: " + e.message;
}
...
MM.UI.Backend.WebDAV.save = function() {
	...
	if (url.match(/\.mmp$/)) { /* complete file name */
	} else { /* just a path */
...
MM.Mouse.handleEvent = function(e) {
	switch (e.type) {
		...
		case "mousedown":
			...
			var item = MM.App.map.getItemFor(e.target); //fixed by mcfee 03/26
			if (item) { MM.App.select(item); }
...
MM.App = {
		...
		switch (message) {
			...
			case "item-change":
				...
				if (publisher.isRoot() && publisher.getMap() == this.map) {
					//document.title = this.map.getName() + " :: Untitled";
				}
			break;
		}
	},
	...
	init: function() {
		...
		this.resource = new MM.UI.Resource();
		...
		if(this._nav)
			this._nav.addEventListener("click", this);
	...
	_syncPort: function() {
		this.portSize = [window.innerWidth - this.ui.getWidth(), window.innerHeight-52];
