 ...
 <table class="FormTable Y_alter" style="width:100%;" id="25ccf24c-9c73-44a5-899b-2565511d81d0" path="Price/eyxhzqmm" code="eyxhzqmm">
  ...
  <tbody>
    <tr class="td-container tr-fixed">
        <td class="text-left" rowspan="1" colspan="6" style="padding-left:0;width:100% !important;">
			<div style="padding-top:20px;" >
                <div style="padding-left: 3px; background-color: #f27302;">
				    <p style="background-color: white; padding-left: 8px; height: 20px; line-height: 20px; margin-bottom: 25px;">应付对账信息</p>
			    </div>
            </div>
        </td>
		 <td colspan="8" style="border:0;">
			...
			<div class="toolbar">
			  <div style="text-align:right;" class="button_workflow">
				<a class="btn btn-red footKeepBtn" href="javascript:void(0);" onclick="SelectOrder(this)">保存&nbsp;<span class="glyphicon glyphicon-floppy-disk"></span></a>
				<!--<a class="btn btn-red footKeepBtn" href="javascript:void(0);" onclick="PrintArea()">打印&nbsp;<span class="glyphicon glyphicon-print"></span></a>
				<a class="btn btn-red footKeepBtn output" href="" target="_blank">导出&nbsp;<span class="glyphicon glyphicon-new-window"></span></a>-->
     ...
     <tr>
        <td rowspan="1" colspan="14" style="background-color: white; border: 0; height: 64px;"></td>
    </tr>
    <tr style="height: 26px;">
		<td class="td_name" style="background-color: rgb(242, 242, 242);">
			 ...
			 <input type="hidden" name="BillType" value="2"/>		<!--1:应收 2:应付-->
      </td>
      <td class="" rowspan="1" colspan="13">
	 ...
	 <tr style="height: 26px;">
       <td class="td_name">
         <a href="javascript:void(0);" onclick="indexOrder(this)" class="edit customer" f-options="{'code':'CompanyID','type':'dialogue','etype':'editable','len':'50','cls':'frame','url':'SupplierSelector.aspx','vals':'Supplierid=id,SupplierName=CompanyName','modalWindow':'1'}" verify="{}">
             承运方
			 <input type="hidden" name="Supplierid" value="-1"/>	<!-- -1:区分筛选自营的订单 -->
			 
         </a>
      </td>
      <td class="" rowspan="1" colspan="4">
         <input name="SupplierName" readonly class="edit form-control transparent" f-options="{'code':'SupplierName','type':'text','etype':'editable','len':'50'}" verify="{'title':'SupplierName','length':'50','required':true}">
      </td>
	...
	<tr style="height: 26px;" class="show_hu">
      ...
      <td class="" rowspan="1" colspan="4">
         <input name="Amount"   placeholder=""  class="edit form-control transparent show_hu_3" f-options="{'code':'Amount','type':'text','etype':'editable','len':'50'}" verify="{}">
      </td>
    </tr>
	<tr style="height: 26px;">
      <td class="" style="background-color: rgb(242, 242, 242);">说明</td>
      <td rowspan="1" colspan="13" style="height:100px;">
	 ...
	 <tr>
		<td rowspan="1" colspan="14" style="background-color: white; border: 0; height: 15px;"></td>
	</tr>
	<tr style="height: 26px;" class="" fb-options="{'rowid':'','initialRows':'','maxrows':'','content':''}">
		...
		<!--<td class="td_name">零担费</td>
		<td class="td_name">整车费</td>
		<td class="td_name">提货费</td>
		<td class="td_name">送货费</td>
		<td class="td_name">装货费</td>
		<td class="td_name">卸货费</td>
		<td class="td_name">最低收费</td>
		<td class="td_name">附加费</td>
		<td class="td_name">保险费</td>-->
		...
		<td class="td_name" style="text-align: right;">
			<a href="javascript:void(0);" tar="Balance_BillDetails" name="add" class="button edit" executecode="%23add%23" f-options="{'code':'add','type':'_button','etype':'','len':''}" verify="{}">新增</a>
		</td>
	</tr>
	<tr nrowkey="DetailID"   style="height: 26px;display:none" class="" fb-options="{'rowid':'Balance_BillDetails','initialRows':'1','maxrows':'10','content':''}" rowid="Balance_BillDetails">
		<td colspan="">
			<input oc="text"  name="Code" placeholder="点击进行订单选择" onclick="showWindow_before(this,'订单选择',$('input[name=\'Supplierid\']'),'',Balance_BillDetails)" title="订单编号" oc="dialogue" class="edit" f-options="{'code':'Code','type':'dialogue','etype':'editable','len':'50','cls':'frame','url':'AccountPayOrderSelect.aspx','vals':'pactcode=pactcode,payment=payment,payable=payable,fromtime=fromtime,Code=code,OrderID=id,fromprovincename=fromprovincename,fromcityname=fromcityname,toprovincename=toprovincename,tocityname=tocityname,volume=Volume,weight=Weight,amount=Amount,receiptqty=ReceiptQty,total=Total','modalWindow':'1'}" verify="{'title':'Code','length':'50','required':true}">
		...
		<td>
			<input name="fromcityname" title="发站市" readonly oc="text" class="edit form-control transparent" f-options="{'code':'fromcityname','type':'text','etype':'editable','len':'50'}" verify="{}">
		</td>
		<td class="">
			<input name="tocityname" readonly="readonly"   title="到站市" oc="number" class="edit form-control transparent" f-options="{'code':'tocityname','type':'number','etype':'editable','len':''}" verify="{}">
		</td>
		<td class="">
			<input  name="volume" readonly="readonly"   title="总体积" oc="number" class="edit form-control transparent" f-options="{'code':'volume','type':'number','etype':'editable','len':''}" verify="{'title':'volume','number':true}">
		</td>
		<td class="">
			<input name="weight" readonly="readonly"   title="总重量" oc="number" class="edit form-control transparent" f-options="{'code':'weight','type':'number','etype':'editable','len':''}" verify="{'title':'weight','number':true}">
		</td>
		<td class="">
		...
		<td>
			<input name="payable" readonly="readonly"   title="货到付款" oc="number" class="edit form-control transparent" f-options="{'code':'total','type':'number','etype':'editable','len':''}" verify="{'title':'货到付款','number':true}">
		</td>
		<td>
			<input name="payment" readonly="readonly"   title="预付款" oc="number" class="edit form-control transparent" f-options="{'code':'total','type':'number','etype':'editable','len':''}" verify="{'title':'预付款','number':true}">
		</td>		
		<td class="">
	   ...
	   <td name="button" class="" style="text-align: right;">
			<a href="javascript:void(0);" tar="Balance_BillDetails" class="button edit"  executecode="%23delete%23"  f-options="{'code':'','type':'_button','etype':'','len':''}" verify="{'title':'','length':'','textarea1':true}">删除</a>
		</td>
...
<script type="text/javascript">
	...
	function PrintArea()
	{
    	...
    	window.location.href="printAccountPay.aspx?id="+_orderid;
     }
...
<script type="text/javascript">
    ...
    function indexOrder(aobj) {
    	...
    	if (_flg) {
    		...
    		$(aobj).attr('onclick', 'showModalWindow(this,"所属承运方")');
    		$(aobj).click();
    	}
    }
</script>
