...
<html>
<head>		
<title>待付订单查询-<%=NDS.Kits.Config.Instance.WebsiteParameters("ProductName")%></title>
<meta charset="utf-8" />
          ...
          <div class="modal-content" style="height:170px">
                <div class="modal-header" style=" padding-bottom:0;padding-top:20px;">
                    ...
                    <div style="padding-left: 3px; background-color: #f27302;">
                        <p class="time_ment zhidan" style="">发货时段</p>
                    </div>
                </h4>
                </div>
                <div class="modal-body">
...
<div class="list-item box jplist-panel tabbtn">
	<div class=""><!-- block right -->
		<table class="jptable table table-border" id="demoList">
			<thead>
				<tr class="trtitle">
					<td class="td_name" style="width:35px;"></td>
			...
			{{each data}}
				<tr>
					...
					<td class="title" fld="suppliername" title="{{$value.suppliername}}">{{$value.suppliername}}</td>
					<td class="title" fld="fromcityname" title="{{$value.fromcityname}}">{{$value.fromcityname}}</td>
				 
					<td class="title" fld="tocityname" title="{{$value.tocityname}}">{{$value.tocityname}}</td>
					<td class="title" fld="Volume" title="{{$value.Volume}}">{{$value.Volume}}</td>
					<td class="title" fld="Weight" title="{{$value.Weight}}">{{$value.Weight}}</td>
					<td class="title" fld="Amount" title="{{$value.Amount}}">{{$value.Amount}}</td>
					<td class="title" fld="ReceiptQty" title="{{$value.ReceiptQty}}">{{$value.ReceiptQty}}</td>
					<td class="title" fld="Total">{{$value.Total}}</td>  
		...
		<div id="demo" class="box jplist">
			...
			<div class="jplist-panel box panel-top min_height">
				...
				<div class="jplist-drop-down" data-control-type="sort-drop-down" data-control-name="sort" data-control-action="sort">
					<ul>
						<li><span data-path="fromtime" data-order="desc" data-type="date">排序</span></li>
					</ul>
				</div>
				...
				<div class="text-filter-box">
					<input data-path="code" data-button="#opt_status-search-button" type="text" value="" placeholder="订单编号" data-control-type="textbox" data-control-name="code" data-control-action="filter"> </div>
				<div class="text-filter-box">
				...
				<div class="text-filter-box">
					<input data-path="rangefilter_fromtime"  class="rangeModal time" name="fromtime" data-button="#search-button" data-target=""  type="text" value="" placeholder="发货时段" data-control-type="textbox" data-control-name="rangefilter_fromtime" data-control-action="filter">					
...
<script type="text/javascript">
    ...
    var reqeustDone = function () {		/*所有JS加载完成以后执行*/
        ...
        InitList($("#demo"), $("#demo .list"), $("#jplist-template").html(), "Index", "/Widget.aspx?param=jplist&vid=grid_jplist_0119");
