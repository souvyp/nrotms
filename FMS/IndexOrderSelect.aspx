...
<body code="TheOrder_Select" style="width:1200px; overflow-x: hidden;">
<!-- 通用对话框开始-->
		...
		<table class="jptable table" id="demoList">
			...
			{{each data}}
				<tr>
					...
					<td class="title" fld="fromprovincename" title="{{$value.fromprovincename}}">{{$value.fromprovincename}}</td>
					<td class="title" fld="fromcityname" title="{{$value.fromcityname}}">{{$value.fromcityname}}</td>
					<td class="title" fld="toprovincename" title="{{$value.toprovincename}}">{{$value.toprovincename}}</td>
					...
					<td class="title" fld="Total" title="{{$value.Total,$value.Min | num}}">
						{{if $value.Total == '0.00'}}
							<span style="color:red">0.00</span>
						{{else}}
							{{$value.Total,$value.Min | num}}
						{{/if}}
					</td>
					<td class="title" fld="payable">{{$value.payable}}</td>
					<td class="title" fld="payment">{{$value.payment}}</td>
					<td class="title" fld="PayAmount">{{$value.PayAmount}}</td>							
				</tr>
			{{/each}}
			</tbody>
		</table>
...
<script type="text/javascript">
    ...
    var reqeustDone = function () {		/*所有JS加载完成以后执行*/
        ...
        InitList($("#demo"), $("#demo .list"), $("#jplist-template").html(), "client", "/Widget.aspx?param=jplist&vid=grid_jplist_0062", false, function () {
            var code = [];
        ...
        template.helper( "num", Total );
        function Total( Total, Min )
        {
        	var _value;
