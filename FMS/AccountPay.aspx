...
<html>
<head>		
<title>应付对账列表-<%=NDS.Kits.Config.Instance.WebsiteParameters("ProductName")%></title>
<meta charset="utf-8" />
		...
		<table class="jptable table table-border" id="demoList">
			...
			{{each data}}
				<tr>
	                ...
	                <td class="title"　 fld="id" title="{{$value.id}}"><a  href="AccountPay_edit.aspx?id={{$value.id}}">{{$value.id}}</a></td>				
					...
					<td class="title">
						<a title="打印" target="_blank" href='printAccountPay.aspx?id={{$value.id}}' >打印</a>
						<a title="导出" target="_blank" href='AccountPayOutput.aspx?id={{$value.id}}' >导出</a>
...
<!--<><><><><><><><><><><><><><><><><><><><><><><><><><> 数据列表开始 <><><><><><><><><><><><><><><><><><><><><><><><><><>-->
		<div id="demo" class="box jplist">
			...
			<div class="jplist-panel box panel-top min_height">
				<button type="button" class="form_sub" onclick="javascript:location.href='AccountPay_edit.aspx'">新增&nbsp;<i class="glyphicon glyphicon-plus-sign"></i></button>
		       ...
		       <div class="text-filter-box"><input data-path="id" data-button="#opt_status-search-button" type="text" value="" placeholder="对账单号" data-control-type="textbox" data-control-name="id" data-control-action="filter"> </div>
				<div class="text-filter-box"><input data-path="suppliername" data-button="#opt_status-search-button" type="text" value="" placeholder="承运方名称" data-control-type="textbox" data-control-name="suppliername" data-control-action="filter"> </div>
...
<script type="text/javascript">
    ...
    var reqeustDone = function () {		/*所有JS加载完成以后执行*/
        ...
        InitList($("#demo"), $("#demo .list"), $("#jplist-template").html(), "AccountPay", "/Widget.aspx?param=jplist&vid=grid_jplist_0065");
