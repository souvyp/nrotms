...
<html>
<head>		
<title>应收结账信息-<%=NDS.Kits.Config.Instance.WebsiteParameters("ProductName")%></title>
<meta charset="utf-8" />
...
<body code="AccountsReport" onload="getdate()">
...
<!--<><><><><><><><><><><><><><><><><><><><><><><><><><> 数据列表开始 <><><><><><><><><><><><><><><><><><><><><><><><><><>-->
		<div id="demo" class="box jplist">
            ...
            <div class="maintitle_container">
                <p class="mainHtml_tit" style="float: left;">应收结账</p>
               	<div style="float: right;" class="btn btn-red footKeepBtn" onclick="PrintArea()">打印&nbsp;<span class="glyphicon glyphicon-print"></span></div>
           <a style="float: right;margin:5px;position:relative;bottom:5px" class="btn btn-red footKeepBtn" target="_blank"  onclick="AccountsReportOutput(this)" id="printbtn">导出&nbsp;<span class="glyphicon glyphicon-new-window"></span></a>
            </div>
			<!-- 操作面板 -->
			<div class="jplist-panel box panel-top min_height">
				<div class="text-filter-box hide">
					...
					<button type="button" id="btn_click" onclick="demo()"><i class="glyphicon glyphicon-search"></i></button>
				</div>
...
<!--#include file="/Controls/RMS/JS.aspx"-->
...
<script>	
	...
	function getData()
	{
		$('tr[name="AccountsReport"]').attr('view-id','{ id:"rms_0014",cross:"false", rowIdentClass:"AccountsReport", paras:[{"name":"Year","value":"'+ Year +'"},{"name":"rows","value":10},{"name":"page","value":1}]}')
		var _AllList = new NSF.System.Data.Grid();		
		_AllList.Pagination("first-ds-pag", $("div[name='first-ds-pag']"),"page");
		_AllList.Initialize("/", "AccountsReport", $("tr[name='AccountsReport']").attr("view-id"), $("tr[name='AccountsReport']"));
		if( !$('tr').hasClass('AccountsReport') )
			$('.jplist-no-results').removeClass('hide');
	}
	
	function page(page) {
		$('tr[name="AccountsReport"]').attr( 'view-id', '{ id:"rms_0014",cross:"false", rowIdentClass:"AccountsReport", paras:[{"name":"Year","value":"'+ Year +'"},{"name":"rows","value":10},{"name":"page","value":'+ page +'}]}');
		getData();
	}
