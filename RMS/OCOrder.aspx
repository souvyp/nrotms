...
<html>
<head>		
<title>客户订单-<%=NDS.Kits.Config.Instance.WebsiteParameters("ProductName")%></title>
<meta charset="utf-8" />
...
<script type="text/javascript">
    var reqeustDone = function () {		/*所有JS加载完成以后执行*/
        ...
        function OcOrder()
        {
                var vml = '[{"namespace":"protocol","cmd":"data","version":1,"id":"rms_0004","paras":[]}]';
                ...
                function ( result, data )
                {
                    if ( result )
                    {
                        ...
                        if ( length != 0 )
                        {
                            ...
                            {
                                _html += '<tr>';
                                _html += '<td title="'+ data[0].rs[0].rows[i].Code +'">' + data[0].rs[0].rows[i].Code + '</td><td title="'+ data[0].rs[0].rows[i].CustomerName +'">' +
                                 data[0].rs[0].rows[i].CustomerName + '</td>';
                                _html += '<td title="'+ data[0].rs[0].rows[i].CreateTime +'">' + data[0].rs[0].rows[i].CreateTime + '</td><td title="'+ data[0].rs[0].rows[i].Volume +'">' + 
                                data[0].rs[0].rows[i].Volume + '</td>';
                        ...
                        {
                            _html += '<tr><td colspan="9" style="text-align:center !important">该时间段没有数据！！！</td></tr>';
                        }
                        $( '.OcoYL' ).append( _html );
                    }
