...
<html>
<head>		
<title>运输订单(自营)-<%=NDS.Kits.Config.Instance.WebsiteParameters("ProductName")%></title>
<meta charset="utf-8" />
...
<!--<><><><><><><><><><><><><><><><><><><><><><><><><><> 数据列表开始 <><><><><><><><><><><><><><><><><><><><><><><><><><>-->
		<div id="demo" class="box jplist">
            ...
            <div class="maintitle_container">
               <p class="mainHtml_tit">运输订单(自营)</p>
            </div>
			<!-- 操作面板 -->
...
<script type="text/javascript">
    var reqeustDone = function () {		/*所有JS加载完成以后执行*/
        ...
        function OsCarrSelf()
        {
            var vml = '[{"namespace":"protocol","cmd":"data","version":1,"id":"rms_0006","paras":[]}]';
            ...
            function ( result, data )
            {
                if ( result )
                {
                    ...
                    if ( length != 0 )
                    {
                        ...
                        {
                            _html += '<tr>';
                            _html += '<td title="'+ data[0].rs[0].rows[i].Code +'">' + data[0].rs[0].rows[i].Code + '</td><td title="'+ data[0].rs[0].rows[i].DriverName +'">' + data[0].rs[0].rows[i].DriverName + '</td><td title="'+ data[0].rs[0].rows[i].CarSN +'">' + data[0].rs[0].rows[i].CarSN + '</td>';
                            ...
                            _html += '<td title="'+ data[0].rs[0].rows[i].FromCityName +'">' + data[0].rs[0].rows[i].FromCityName + '</td><td title="'+ data[0].rs[0].rows[i].ToCityName +'">' + data[0].rs[0].rows[i].ToCityName + '</td>';
                            _html += '<td title="'+ data[0].rs[0].rows[i].StatusName +'">' + data[0].rs[0].rows[i].StatusName + '</td><td title="'+ data[0].rs[0].rows[i].ID +'">' + data[0].rs[0].rows[i].ID + '</td></tr>';
                        }
                    }
                    else
                    {
                        _html += '<tr><td colspan="10" style="text-align:center !important">没有数据！！！</td></tr>';
                    }
                    $( '.OscSYL' ).append( _html );
                }
